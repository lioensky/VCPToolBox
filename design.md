...
- 左右操作台页面要保持"左侧固定、右侧滚动"的感觉: 左侧 console 使用 `position: sticky` 贴住内容视口顶部, 高度按 `app-viewport - topbar - 内容区 padding` 计算; 如果左侧项目很多, 只让 `UiSideConsoleNav` 内部滚动, 不让整个侧栏跟随右侧内容滚走。
