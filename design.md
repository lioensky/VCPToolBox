...
- 左右操作台页面要保持"左侧固定、右侧滚动"的感觉: 页面/grid 层使用固定高度和 `overflow: hidden`, 左侧 console 高度 100% 且不参与页面滚动, 右侧主内容使用 `overflow-y: auto`; 如果左侧项目很多, 只让 `UiSideConsoleNav` 内部滚动, 不让整个侧栏跟随右侧内容滚走。不要只靠 `position: sticky` 修补, 在当前主滚动容器里容易仍被外层滚动带走。
