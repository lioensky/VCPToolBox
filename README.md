...
### 3.10 VCPTaskAssistant 闭环任务中心

VCPTaskAssistant 已从早期的“任务派发中心”升级为有回声、有验收、有记录的闭环任务执行中心。它不再只判断 HTTP 请求是否成功，而是追踪 Agent 是否真正完成任务、是否留下公开痕迹、是否需要继续等待或请求用户确认。

**核心能力：**

- **任务完成语义**：统一使用 `【任务回声】...【任务回声结束】` 协议，让 Agent 在最终回声中声明状态、主动动作、公开痕迹、未来锚点和未决事项。
- **独立运行记录**：任务配置拆分保存到 `Plugin/VCPTaskAssistant/tasks/`，每次执行详情保存到 `Plugin/VCPTaskAssistant/task-runs/`，便于追踪、展开和排障。
- **系统验收摘要**：任务中心会自动检查任务回声、完成状态、主动动作、公开落点、未来锚点与提示词中声明的日记落点。
- **AgentAssistant 异步委托闭环**：支持将任务提交给 AgentAssistant 后后台轮询，已提交或等待回声的任务在重启后也会恢复检查，直到完成、失败或超时。
- **任务草案创建**：Agent 如需自主创建任务，可使用 `createTaskFromDraft` 提交中文任务草案，由任务中心归一化、校验、注册与调度。
- **梦系统安静时段**：支持 `dreamQuietWindow`，避免任务在梦系统安静窗口内直接打断节律，必要时顺延到窗口结束后执行。

**管理端 API：**

| 方法 | 端点 | 说明 |
|------|------|------|
| `GET` / `POST` | `/admin_api/task-assistant/config` | 读取或保存任务中心配置 |
| `GET` | `/admin_api/task-assistant/status` | 查看全局开关、定时器、任务运行态和最近历史 |
| `GET` | `/admin_api/task-assistant/runs` | 查询最近执行记录，可按 `taskId` 过滤 |
| `GET` | `/admin_api/task-assistant/runs/:runId` | 查看单次任务的完整运行详情 |
| `POST` | `/admin_api/task-assistant/trigger` | 手动触发指定任务或指定 Agent 的巡航任务 |
| `GET` / `POST` / `PATCH` / `DELETE` | `/admin_api/task-assistant/tasks` | 任务列表、创建、更新与删除 |
...
- **VCPTaskAssistant**：VCP 任务派发与执行闭环中心
  - 支持周期任务、CRON 任务、一次性任务和手动触发任务
  - 可向指定 Agent、多个 Agent 或随机 Agent 集合派发任务
  - 保存每次 run 的原始响应、任务回声、验收摘要、错误详情和委托 ID
  - 支持 Agent 通过中文任务草案创建新任务，并由任务中心统一校验和调度
  - 与 AgentAssistant 异步委托协同，后台轮询最终结果，重启后继续恢复未完成 run
...
当前 VCP 同时支持静态工具箱和动态工具清单两种提示词注入方式：

- 静态工具箱（如 `{{VCPFileToolBox}}`、`{{VCPSearchToolBox}}`）适合放置核心工具、稳定调用范式和安全偏好，并可通过上下文折叠协议按需展开。
- 动态工具清单 `{{VCPDynamicTools}}` 会从当前已加载的本地/分布式插件中生成轻量工具列表，并按当前上下文展开少量相关工具的完整说明。
- 动态工具只影响“给 AI 看到哪些工具说明”，不绕过插件执行链路、审批系统或插件内部权限校验。
- 当插件数量增长或分布式节点频繁上下线时，动态工具清单比 `{{VCPAllTools}}` 更适合作为长尾工具发现层。
...
- **任务派发中心**：配置 VCPTaskAssistant 的任务列表、目标 Agent、调度方式和提示词模板，支持手动触发、查看运行状态、展开最近 run 的任务回声、系统验收和落点痕迹
