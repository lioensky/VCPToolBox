...
TagMemo “认知浪潮”算法是 VCP 系统中用于 RAG（检索增强生成）的核心进化架构。从 V4/V5 的线性向量检索到能量坍缩，如今在 **V6 版本**彻底迎来了**仿脑拓扑网络**的质变。V6 版本引入了受经典脑仿真启发的 **LIF (Leaky Integrate-and-Fire) 漏积放发脉冲神经模型**，这使得 VCP 不再是一个简单的搜素引擎，而是一个具备**自主产生突触联想与涌现拓扑**的认知实体。
...
- **高级控制指令**：
  ...
  - **`river:「始」text/full「末」` (工具上下文注入/金鱼记忆修复)**：允许工具调用时将当前完整的对话上下文（Context）实时“流向”并注入工具库内部。这使得同步/异步工具不仅能执行当前任务，还能洞察之前发生过的一切细节。
    - `text`: 注入精简纯文本形式的上下文数组（自动转换多模态部分为 Text，保留角色）。
