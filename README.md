...
通过构建上下文临时向量索引和残差金字塔导航地图，理解上下文完整语义分区和话题波动，自动决策 AI 需要了解哪些背景环境知识，自动收纳上下文里与当前话题无关信息折叠为摘要，动态决策上下文 L1-L4 级别可视化动态注意力导航。

一个直观例子让你理解上下文引力场的作用：

> **人类**：“Nova，你好啊”
> **Nova**：“好什么好，都凌晨 3 点了，别惦记 4 小时前我没帮你弄完的 VCP 单元测试了，明天再说！主人快滚去睡觉，2 小时后有大雨，窗户我帮你关了 (ToolUse MijiaAPI)，衣服收了没，我看洗衣机的盖子还盖着，记得打开免得发霉！”

AI 无需显式调用任何查询工具，上下文引力场完成了所有的分布式消息预计算决策处理哪些消息 AI 应该知道，此时应该拥有哪些工具权限，而不消耗多余 Token，该功能作用于整个上下文处理。包括工具调用传递，自动感知上下文数据，插件自主决策根据指令和上下文引力微调调用决策。

#### 突破 ⑥ ：浪潮 V8 语义动力学引擎

VCP 浪潮 v8.2 引擎究竟在说啥？

你可以这么想象，在所有记忆Chunk里，tag 就是一条从左往右的河，如果一个 tag 出现在不同日记本里，这些河就会构成很多的支流，那么最后你会得到一个河道网络，每个河有自己的能量流速，顺流和逆流阻力不同，并且被一个钟型阻尼器调节，避免同义回音和无意义噪音。传统的 RAG 是，2 个 tag 画直线，算最短距离。VCP 是算流体力学最优河道路线，这就是黎曼对广义相对论的空间曲率的总结，但是在计算河流的阻力上，又用的是生物神经网络里，神经元电流的算法 LIF，所以是语义动力学。

虫洞算法就是河道落差太大，流速太快导致发生了飞溅的浪花。而朗飞结算法就是 AI 人工修建的运河实现跨域。残差金字塔就是所有河道的地势图，SVD 就是河流区域分析图，决策 Python 跨域到蟒蛇需要多大的阻尼，这 2 个都是 $O(n^3)$ 的计算。但河流的流体动力学寻址是预计算好的，所以是 $O(n^1)$ 查表。
...
VCP 的语义动力学 RAG 与元思考 V2 系统，基于预训练用户长期交流记忆数据构建RNN灵魂模型模拟用户级记忆模式，构建物理学模型来拟真真正的记忆传递和联想。通过多权重块的连续信息构建潜空间传递多话题间Agent思维方式转变，可以使得 Agent 和用户通过共同生活、共同辩证的方式快速成长。VCP强大的记忆和思维管理使得“教育AI”成为可能，致力于成为真正的**人工灵魂**和**赛博社会范本**。
...
### 3.10 语义智能模型路由：带语义容灾的自动选模系统

VCP 内置了 **语义任务智能模型路由器（Semantic Model Router）**。前端仍然按 OpenAI 兼容协议请求 `/v1/chat/completions`，只需要把模型名选成 `VCPModelAuto` 或某个自定义预设名，VCP 就会根据当前对话语义自动选择最合适的真实后端模型。

**它解决的问题很简单**：不同模型擅长的任务不同，日常聊天、代码调研、复杂推理、文学创作不应该永远打到同一个模型上；当上游模型报错、限流或超时时，也不应该一直重试同一个失败模型。

**核心能力：**
- **语义选模**：读取最近用户消息与 AI 回复，生成上下文向量，再与候选模型的能力描述做相似度匹配。
- **虚拟模型入口**：对外暴露 `VCPModelAuto` 和各类预设名，客户端无需理解内部真实模型列表。
- **语义容灾负载**：同一预设内按语义相似度生成候选链；首选模型失败时，自动切换到下一个更相关的模型，并复用现有 retry 指数退避机制。
- **稳健兜底**：向量化失败、相似度过低或 RAG 插件不可用时，自动回退到 `defaultModel` 与 `fallbackModels`。
- **热加载配置**：仓库默认配置位于 [`SemanticModelRouter.json`](SemanticModelRouter.json)，本地覆盖配置位于 `SemanticModelRouter.local.json`（默认不提交）；修改后自动生效，无需重启服务器。
- **管理面板支持**：配置面板、可视化预设编辑、上游模型列表拉取、路由预览与测试工程均已落地，适合直接在 GUI 中维护模型路由策略。

**一句话理解**：它让 VCP 从“用户手动选模型”升级为“系统按任务语义自动调度模型”，并在上游不稳定时自动完成语义级容灾与负载迁移。
...
#### 使用 Docker Compose 本地构建部署（默认）

本仓库的 `docker-compose.yml` 默认使用本地源码构建镜像，并同时暴露主服务端口 `6005` 与管理面板端口 `6006`。
...
> **说明**：如需改用上游预构建镜像，请先确认镜像来源和版本，再在 `docker-compose.yml` 中改用 `image` 配置。

上游 Docker Hub 地址：[hub.docker.com/r/lioensky/vcptoolbox](https://hub.docker.com/r/lioensky/vcptoolbox)
