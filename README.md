...
### 4.1 TagMemo V6: LIF Spike Propagation

TagMemo “认知浪潮”算法是 VCP 系统中用于 RAG（检索增强生成）的核心进化架构。从 V4/V5 的线性向量检索到能量坍缩，如今在 **V6 版本**彻底迎来了**仿脑拓扑网络**的质变。V6 版本引入了受果蝇全脑仿真启发的 **LIF (Leaky Integrate-and-Fire) 漏积放发脉冲神经模型**，这使得 VCP 不再是一个简单的搜素引擎，而是一个具备**自主产生突触联想与涌现拓扑**的认知实体。

[深入了解 TagMemo V6 深度技术文档](TagMemo_Wave_Algorithm_Deep_Dive.md)

#### 核心哲学：拓扑连接主义与脉冲扩散

在 V6 的视角下，单个 Tag 节点的内容只是载体，**真正的逻辑孕育在 Tag 与 Tag 共同编织的拓扑网络中**：

- **LIF 神经元**：标签（Tags）被视为空间中的神经元节点。
- **初始电位注入**：原始 Query 作为初始电流，注入命中的种子神经元。
- **N-Hop 脉冲扩散**：电流顺着系统日积月累生成的**共现矩阵（Connectome）**向更深层的邻居传递，带来出乎意料但合乎逻辑的跨越性联想。
