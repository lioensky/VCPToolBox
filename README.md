...
- `{{Today}}`：当天星期几（中文）
- `{{Festival}}`：农历日期、生肖、节气
- `{{VCPDistributedServerList}}`：当前已连接的 VCP 分布式服务器列表，包含节点自报名称、中央分配的 `serverId`、`clientId`、公网 IP、局域网 IP、本节点注册工具列表、连接时间与最近上报时间。该变量会随分布式节点连接、断开、工具注册与 IP 上报自动反映最新状态，可用于让 AI 在顶层系统提示词或 `SarPrompt*` 等嵌套变量中持续感知当前可用的分布式节点。
