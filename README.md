...
Docker 部署默认会通过 `pm2-runtime` 在同一容器内托管两个进程：

- `vcp-main -> server.js`，监听主端口（如 `6005`）
- `vcp-admin -> adminServer.js`，监听主端口 + 1（如 `6006`）

因此 `docker-compose.yml` 需要同时暴露这两个端口。
...
- `VCPChat` 仍继续连接主端口，例如 `http://<您的服务器IP或域名>:<端口>/v1/chat/completions`
