...
class SearchCache {
  ...
  getCacheKey(diaryName, queryVector, k) {
    const vectorHash = crypto.createHash('md5').update(Buffer.from(queryVector)).digest('hex');
...
class VectorDBManager {
  ...
  recordMetric(type, duration) {
    if (type === 'search_success') {
      ...
      this.stats.avgSearchTime =
        (this.stats.avgSearchTime * (this.stats.totalSearches - 1) + duration) / this.stats.totalSearches;
    }
  }
  ...
  async scanAndSyncAll() {
    try {
      // 确保 dailynote 目录存在
      await fs.mkdir(DIARY_ROOT_PATH, { recursive: true });
      ...
      if (error.code === 'ENOENT') {
        console.log(`[VectorDB] Diary root path does not exist, skipping scan: ${DIARY_ROOT_PATH}`);
      } else {
        console.error('[VectorDB] Error during scanAndSyncAll:', error);
      }
    }
  }
  ...
  async calculateChanges(diaryName) {
    ...
    const oldChunkHashToLabel = new Map(
      Object.entries(oldChunkMap).map(([label, data]) => [data.chunkHash, Number(label)]),
    );
  ...
  async scheduleDiaryBookProcessing(diaryName) {
    ...
    try {
      ...
      if (totalOldChunks === 0 || changeRatio > this.config.changeThreshold) {
        console.log(
          `[VectorDB] Major changes detected (${(changeRatio * 100).toFixed(
            1,
          )}%). Scheduling a full rebuild for "${diaryName}".`,
        );
    ...
    return new Promise((resolve, reject) => {
      ...
      worker.on('message', message => {
        ...
        if (message.status === 'success') {
          ...
          this.recordMetric('search_success', performance.now() - startTime);
          console.log(
            `[VectorDB][Search] Worker found ${searchResults.length} matching chunks for "${diaryName}". Resolving promise.`,
          );
  ...
  async preWarmIndices() {
    ...
    const preLoadPromises = sortedDiaries.slice(0, preLoadCount).map(diaryName => this.loadIndexForSearch(diaryName));
