...
const server = app.listen(ADMIN_PORT, "0.0.0.0", () => {
  ...
  if (process.send) {
    process.send("ready");
    console.log('[AdminServer] Sent "ready" signal to PM2.');
  }

  // 挂载心跳保活（每1小时执行一次，防止 Node.js 事件循环为空导致自动退出）
  setInterval(() => {
    if (DEBUG_MODE) console.log("[AdminServer] Heartbeat keep-alive.");
  }, 3600000);
});

server.on("error", (error) => {
  console.error("[AdminServer] 🚨 HTTP Server Error:", error);
});
