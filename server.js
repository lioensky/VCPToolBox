...
// 解析 PORT：如果环境变量中有逗号分隔的列表，取第一个有效的数字
let parsedPort = 3000;
if (process.env.PORT) {
  const portMatch = String(process.env.PORT).split(",")[0].trim();
  parsedPort = parseInt(portMatch, 10) || 3000;
}
process.env.PORT = parsedPort.toString(); // 确保后续所有依赖 process.env.PORT 的代码都拿到单值
...
async function closeHttpServerGracefully() {
  ...
  console.log(
    `[Server] Preparing to close HTTP server. trackedSockets=${trackedSockets.size}, activeHttpRequests=${activeHttpRequests.size}`
  );
    ...
    try {
      server.close((error) => {
        if (error) {
          ...
          console.log(
            "[Server] HTTP server stopped accepting new connections."
          );
        }
        resolve();
      });
...
async function waitForActiveRequestsToDrain(timeoutMs = 30000) {
  ...
  while (activeRequests.size > 0 && Date.now() - start < timeoutMs) {
    console.log(
      `[Shutdown] Waiting for ${activeRequests.size} active request(s) to finish...`
    );
...
async function abortAllActiveRequests(reason = "服务器正在关闭") {
  console.log(
    `[Shutdown] Aborting ${activeRequests.size} active request(s)...`
  );
    ...
    try {
      ...
      if (context.res && !context.res.writableEnded && !context.res.destroyed) {
        ...
        if (!context.res.headersSent) {
          ...
          String(context.res.getHeader("Content-Type") || "").includes(
            "text/event-stream"
          )
        ) {
          context.res.write("data: [DONE]\n\n");
      ...
      console.error(
        `[Shutdown] Failed to abort active request ${id}:`,
        error.message
      );
      try {
        ...
        console.error(
          `[Shutdown] Failed to destroy response for request ${id}:`,
...
setInterval(() => {
  ...
  for (const [id, context] of activeRequests.entries()) {
    ...
    if (now - (context.timestamp || 0) > 30 * 60 * 1000) {
      console.log(
        `[Request Cleanup] Aborting and removing timed-out request: ${id}`
      );
...
const CHAT_LOG_ENABLED =
  (process.env.CHAT_LOG_ENABLED || "false").toLowerCase() === "true";
const VCPToolCode =
  (process.env.VCPToolCode || "false").toLowerCase() === "true"; // 新增：读取VCP工具调用验证码开关
const SHOW_VCP_OUTPUT =
  (process.env.ShowVCP || "False").toLowerCase() === "true"; // 读取 ShowVCP 环境变量
const RAG_MEMO_REFRESH =
  (process.env.RAGMemoRefresh || "false").toLowerCase() === "true"; // 新增：RAG日记刷新开关
const ENABLE_ROLE_DIVIDER =
  (process.env.EnableRoleDivider || "false").toLowerCase() === "true"; // 新增：角色分割开关
const ENABLE_ROLE_DIVIDER_IN_LOOP =
  (process.env.EnableRoleDividerInLoop || "false").toLowerCase() === "true"; // 新增：循环栈角色分割开关
const ROLE_DIVIDER_SYSTEM =
  (process.env.RoleDividerSystem || "true").toLowerCase() === "true"; // 新增：System角色分割开关
const ROLE_DIVIDER_ASSISTANT =
  (process.env.RoleDividerAssistant || "true").toLowerCase() === "true"; // 新增：Assistant角色分割开关
const ROLE_DIVIDER_USER =
  (process.env.RoleDividerUser || "true").toLowerCase() === "true"; // 新增：User角色分割开关
const ROLE_DIVIDER_SCAN_SYSTEM =
  (process.env.RoleDividerScanSystem || "true").toLowerCase() === "true"; // 新增：System角色扫描开关
const ROLE_DIVIDER_SCAN_ASSISTANT =
  (process.env.RoleDividerScanAssistant || "true").toLowerCase() === "true"; // 新增：Assistant角色扫描开关
const ROLE_DIVIDER_SCAN_USER =
  (process.env.RoleDividerScanUser || "true").toLowerCase() === "true"; // 新增：User角色扫描开关
const ROLE_DIVIDER_REMOVE_DISABLED_TAGS =
  (process.env.RoleDividerRemoveDisabledTags || "true").toLowerCase() ===
  "true"; // 新增：禁用标签清除开关

let ROLE_DIVIDER_IGNORE_LIST = [];
try {
  ROLE_DIVIDER_IGNORE_LIST = JSON.parse(
    process.env.RoleDividerIgnoreList || "[]"
  );
} catch (e) {
...
let CHINA_MODEL_1 = [];
try {
  CHINA_MODEL_1 = (process.env.ChinaModel1 || "")
    .split(",")
    .map((m) => m.trim())
    .filter((m) => m !== "");
} catch (e) {
...
async function writeDebugLog(filenamePrefix, data) {
  if (DEBUG_MODE) {
    ...
    try {
      ...
      console.log(
        `[DebugLog] 已记录日志: archive/${dateStr}/Debug/${filename}`
      );
    } catch (error) {
...
if (CHAT_LOG_ENABLED) {
  ...
  writeChatLog = function (requestBody, logs) {
    ...
    const id =
      (requestBody && (requestBody.requestId || requestBody.messageId)) ||
      "no-id";
...
async function loadBlacklist() {
  try {
    ...
    console.log(
      `[Security] IP黑名单加载成功，共 ${ipBlacklist.length} 个条目。`
    );
  } catch (error) {
...
app.use((req, res, next) => {
  if (req.method === "POST") {
    ...
    if (serverName) {
      console.log(
        `[IP Tracker] SUCCESS: Post request is from known Distributed Server: '${serverName}' (IP: ${clientIp})`
      );
    }
  }
  next();
...
function handleApiError(req) {
  ...
  if (clientIp === "127.0.0.1" || clientIp === "::1") {
    console.log(
      `[Security] Ignored an API error from the local server itself (IP: ${clientIp}). This is to prevent self-blocking.`
    );
    return;
  }
  ...
  console.log(
    `[Security] IP ${clientIp} 出现API错误，当前计次: ${currentErrors}/${MAX_API_ERRORS}`
  );
...
app.use((req, res, next) => {
  if (isServerDraining()) {
    ...
    if (!allowDuringShutdown) {
      console.warn(
        `[Server] Rejecting new request during ${serverLifecycleState}: ${req.method} ${req.path}`
      );
...
const adminAuth = (req, res, next) => {
  ...
  const isAdminPath =
    req.path.startsWith("/admin_api") || req.path.startsWith("/AdminPanel");
  ...
  if (isAdminPath) {
    ...
    const isReadOnlyPath = readOnlyDashboardPaths.some((path) =>
    ...
    if (!ADMIN_USERNAME || !ADMIN_PASSWORD) {
      console.error(
        "[AdminAuth] AdminUsername or AdminPassword not set in config.env. Admin panel is disabled."
      );
      // 对API和页面请求返回不同的错误格式
      if (
        ...
        res.status(503).json({
          ...
          message:
            "Please set AdminUsername and AdminPassword in the config.env file to enable the admin panel.",
        });
      } else {
        res
          .status(503)
          .send(
            "<h1>503 Service Unavailable</h1><p>Admin credentials (AdminUsername, AdminPassword) are not configured in config.env. Please configure them to enable the admin panel.</p>"
          );
      }
      return; // 停止进一步处理
    }
    ...
    if (blockInfo && Date.now() < blockInfo.expires && !isReadOnlyPath) {
      console.warn(
        `[AdminAuth] Blocked login attempt from IP: ${clientIp}. Block expires at ${new Date(
          blockInfo.expires
        ).toLocaleString()}.`
      );
    ...
    if (
      ...
      if (clientIp && !isReadOnlyPath && isActiveLoginAttempt) {
        ...
        attemptInfo.count++;
        console.log(
          `[AdminAuth] Failed login attempt from IP: ${clientIp}. Count: ${attemptInfo.count}/${MAX_LOGIN_ATTEMPTS}`
        );

        if (attemptInfo.count >= MAX_LOGIN_ATTEMPTS) {
          console.warn(
            `[AdminAuth] IP ${clientIp} has been temporarily blocked for ${
              TEMP_BLOCK_DURATION / 60000
            } minutes due to excessive failed login attempts.`
          );
      ...
      // 🌟 防DDoS：无凭据访问独立计数，阈值更宽松（不影响正常 cookie 过期场景）
        ...
        if (accessInfo.count >= MAX_NO_CREDENTIAL_REQUESTS) {
          console.warn(
            `[AdminAuth] IP ${clientIp} blocked for ${
          ...
          if (accessInfo.count % 10 === 0) {
            console.log(
              `[AdminAuth] Unauthenticated access from IP: ${clientIp}. Count: ${accessInfo.count}/${MAX_NO_CREDENTIAL_REQUESTS}`
            );
          }
        }
      }
      ...
      // 其他情况
        ...
        return res
          .status(401)
          .send(
            "<h1>401 Unauthorized</h1><p>Authentication required to access the Admin Panel.</p>"
          );
      }
      // ========== 修改结束 ==========
    }
...
app.get("/v1/models", async (req, res) => {
  ...
  const appendSemanticRouterModels = (modelsData) => {
    ...
    const existingIds = new Set(
      modelsData.data.map((model) => model && model.id).filter(Boolean)
    );
  ...
  try {
    ...
    if (apiResponse.ok) {
      ...
      try {
            ...
            modelsData.data = modelsData.data.map((model) => {
              if (model.id) {
                ...
                if (publicModelName !== model.id) {
                  if (DEBUG_MODE) {
                    console.log(
                      `[ModelRedirect] 模型列表重定向: ${model.id} -> ${publicModelName}`
                    );
                  }
        ...
        console.warn(
          "[Models] 解析模型列表响应失败，返回语义路由虚拟模型列表:",
          parseError.message
        );
        const fallbackModelsData = appendSemanticRouterModels({
          object: "list",
          data: [],
        });
...
app.post("/v1/schedule_task", async (req, res) => {
  ...
  if (isNaN(targetDate.getTime())) {
    return res
      .status(400)
      .json({ status: "error", error: "无效的 'schedule_time' 时间格式。" });
  }
  if (targetDate.getTime() <= Date.now()) {
    return res
      .status(400)
      .json({ status: "error", error: "schedule_time 不能是过去的时间。" });
  }
  ...
  try {
    ...
    if (DEBUG_MODE)
      console.log(`[Server] 已通过API创建新的定时任务文件: ${taskFilePath}`);
...
app.post("/v1/interrupt", (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ error: "requestId or messageId is required." });
  }
  ...
  if (context) {
    ...
    if (!context.aborted) {
      ...
      setImmediate(() => {
        ...
        if (
          context.res &&
          !context.res.writableEnded &&
          !context.res.destroyed
        ) {
          try {
            // 检查响应头是否已发送，决定如何关闭
            if (!context.res.headersSent) {
              ...
              if (isStreamRequest) {
                // 流式请求：发送SSE格式的中止信号
                console.log(
                  `[Interrupt] Sending SSE abort signal for stream request ${id}`
                );
                context.res.status(200);
                context.res.setHeader("Content-Type", "text/event-stream");
                context.res.setHeader("Cache-Control", "no-cache");
                ...
                // 非流式请求：发送标准JSON响应
                console.log(
                  `[Interrupt] Sending JSON abort response for non-stream request ${id}`
                );
              ...
              context.res.end();
              console.log(
                `[Interrupt] Sent [DONE] signal and closed stream for ID: ${id}`
              );
            } else {
              // 其他情况，直接结束响应
              context.res.end();
            ...
            console.error(
              `[Interrupt] Error closing response for ${id}:`,
              e.message
            );
            // 即使写入失败也不要崩溃，只记录错误
            // 尝试强制关闭连接以防止挂起
            try {
              if (context.res && !context.res.destroyed) {
                context.res.destroy();
                console.log(
                  `[Interrupt] Forcefully destroyed response for ${id}`
                );
              }
            } catch (destroyError) {
              console.error(
                `[Interrupt] Error destroying response for ${id}:`,
                destroyError.message
              );
            }
          }
          ...
          console.log(
            `[Interrupt] Response for ${id} already closed or destroyed.`
          );
        }
      });
    } else {
      console.log(
        `[Interrupt] Request ${id} already aborted, skipping duplicate abort.`
      );
    }
...
app.post("/v1/chat/completions", async (req, res) => {
  try {
    ...
    console.error(
      `[FATAL] Uncaught exception from chatCompletionHandler for ${req.path}:`,
      e
    );
    if (!res.headersSent) {
...
app.post("/v1/human/tool", async (req, res) => {
  try {
    ...
    if (typeof requestBody !== "string" || !requestBody.trim()) {
      return res
        .status(400)
        .json({ error: "Request body must be a non-empty plain text." });
    }
    ...
    if (!extractedToolBlock) {
      return res
        .status(400)
        .json({ error: "Malformed request: Missing TOOL_REQUEST markers." });
    }

    const parsedToolCall = ToolCallParser.parseBlock(
      extractedToolBlock.blockContent
    );
    ...
    if (DEBUG_MODE) {
      console.log(
        `[Human Tool Exec] Received tool call for: ${requestedToolName}`,
        parsedToolArgs
      );
    }
    ...
    console.error(
      "[Human Tool Exec] Error processing direct tool call:",
      error.message
    );
...
async function handleDiaryFromAIResponse(responseText) {
  ...
  if (fullAiResponseTextForDiary.trim()) {
    ...
    if (match && match[1]) {
      ...
      if (DEBUG_MODE)
        console.log(
          "[handleDiaryFromAIResponse] Found structured daily note block."
        );
      ...
      if (maidName && dateString && contentText) {
        ...
        try {
          if (DEBUG_MODE)
            console.log(
              "[handleDiaryFromAIResponse] Calling DailyNoteWrite plugin with payload:",
              diaryPayload
            );
          ...
          if (
            ...
            if (DEBUG_MODE)
              console.log(
                `[handleDiaryFromAIResponse] DailyNoteWrite plugin reported success: ${dailyNoteWriteResponse.message}`
              );
            ...
            const notification = {
              ...
              data: {
                ...
                filePath: filePath,
                status: "success",
                message: `日记 '${filePath || "未知路径"}' 已为 '${
                  diaryPayload.maidName
                }' (${diaryPayload.dateString}) 创建成功。`,
              },
            };
            webSocketServer.broadcast(notification, "VCPLog");
            if (DEBUG_MODE)
              console.log(
                "[handleDiaryFromAIResponse] Broadcasted daily_note_created notification:",
                notification
              );
          } else if (pluginResult && pluginResult.status === "error") {
            // Handle errors reported by the plugin's JSON response
            console.error(
              `[handleDiaryFromAIResponse] DailyNoteWrite plugin reported an error:`,
              pluginResult.message || pluginResult
            );
          } else {
            // Handle cases where pluginResult is null, or status is not "success"/"error", or message is missing on success.
            console.error(
              `[handleDiaryFromAIResponse] DailyNoteWrite plugin returned an unexpected response structure or failed:`,
              pluginResult
            );
          }
        } catch (pluginError) {
          ...
          console.error(
            "[handleDiaryFromAIResponse] Error executing DailyNoteWrite plugin:",
            pluginError.message,
            pluginError.stack
          );
        }
        ...
        console.error(
          "[handleDiaryFromAIResponse] Could not extract Maid, Date, or Content from daily note block:",
          { maidName, dateString, contentText: contentText?.substring(0, 50) }
        );
      }
    }
  }
...
const adminPanelRoutes = require("./routes/adminPanelRoutes")(
  ...
  (code = 1) => {
    console.log(
      `[Server] Restart triggered from admin API (exit code: ${code}).`
    );
...
async function ensureAsyncResultsDir() {
  try {
    ...
    console.error(
      `[ServerSetup] 创建 VCPAsyncResults 目录失败: ${VCP_ASYNC_RESULTS_DIR}`,
      error
    );
  }
}
...
app.post("/plugin-callback/:pluginName/:taskId", async (req, res) => {
  ...
  if (DEBUG_MODE) {
    console.log(
      `[Server] Received callback for plugin: ${pluginName}, taskId: ${taskId}`
    );
    console.log(
  ...
  const resultFilePath = path.join(
    VCP_ASYNC_RESULTS_DIR,
    `${pluginName}-${taskId}.json`
  );
  try {
    await fs.writeFile(
      resultFilePath,
    ...
    if (DEBUG_MODE)
      console.log(
        `[Server Callback] Saved async result for ${pluginName}-${taskId} to ${resultFilePath}`
      );
  } catch (fileError) {
    console.error(
      `[Server Callback] Error saving async result file for ${pluginName}-${taskId}:`,
      fileError
    );
  ...
  if (pluginManifest.webSocketPush && pluginManifest.webSocketPush.enabled) {
    const targetClientType =
      pluginManifest.webSocketPush.targetClientType || null;
    const wsMessage = {
      type:
        pluginManifest.webSocketPush.messageType ||
        "plugin_callback_notification",
      data: callbackData,
    };
    webSocketServer.broadcast(wsMessage, targetClientType);
    if (DEBUG_MODE) {
      console.log(
        `[Server Callback] WebSocket push for ${pluginName} (taskId: ${taskId}) processed. Message:`,
        JSON.stringify(wsMessage, null, 2)
      );
    }
  } else if (DEBUG_MODE) {
    console.log(
      `[Server Callback] WebSocket push not configured or disabled for plugin: ${pluginName}`
    );
  }

  res
    .status(200)
    .json({ status: "success", message: "Callback received and processed" });
});
...
async function initialize() {
  ...
  console.log(
    "服务类插件初始化完成，管理面板 API 路由和 VCP 论坛 API 路由已挂载。"
  );
  ...
  try {
    ...
    if (DEBUG_MODE)
      console.log("[Server] Injecting dependencies into plugins...");
    ...
    for (const [name, module] of pluginManager.messagePreprocessors) {
      if (typeof module.setDependencies === "function") {
        ...
        if (DEBUG_MODE)
          console.log(
            `  - Injected dependencies into message preprocessor: ${name}.`
          );
      }
    }
    // 注入到服务模块 (排除VCPLog自身)
    for (const [name, serviceData] of pluginManager.serviceModules) {
      if (
        name !== "VCPLog" &&
        typeof serviceData.module.setDependencies === "function"
      ) {
        serviceData.module.setDependencies(dependencies);
        if (DEBUG_MODE)
          console.log(`  - Injected dependencies into service: ${name}.`);
      }
    }
  } catch (e) {
  ...
  if (DEBUG_MODE)
    console.log(
      "开始从插件目录加载表情包列表到缓存 (由EmojiListGenerator插件生成)..."
    );
  const emojiListSourceDir = path.join(
    __dirname,
    "Plugin",
    "EmojiListGenerator",
    "generated_lists"
  ...
  try {
    ...
    const txtFiles = listFiles.filter((file) =>
    ...
    if (txtFiles.length === 0) {
      if (DEBUG_MODE)
        console.warn(
          `[initialize] Warning: No .txt files found in emoji list source directory: ${emojiListSourceDir}`
        );
    } else {
      if (DEBUG_MODE)
        console.log(
          `[initialize] Found ${txtFiles.length} emoji list files in ${emojiListSourceDir}. Loading...`
        );
      await Promise.all(
        txtFiles.map(async (fileName) => {
          ...
          try {
            ...
            console.error(
              `[initialize] Error reading emoji list file ${filePath}:`,
              readError.message
            ); // Keep as error
            cachedEmojiLists.set(
              emojiName,
              `[加载 ${emojiName} 列表失败: ${readError.code}]`
            );
          }
        })
      );
      if (DEBUG_MODE)
        console.log(
          "[initialize] All available emoji lists loaded into cache."
        );
    }
  } catch (error) {
    if (error.code === "ENOENT") {
      console.error(
        `[initialize] Error: Emoji list source directory not found: ${emojiListSourceDir}. Make sure the EmojiListGenerator plugin ran successfully.`
      ); // Keep as error
    } else {
      console.error(
        `[initialize] Error reading emoji list source directory ${emojiListSourceDir}:`,
        error.message
      ); // Keep as error
    }
  }
...
async function startServer() {
  ...
  await modelRedirectHandler.loadModelRedirectConfig(
    path.join(__dirname, "ModelRedirect.json")
  );
  console.log("模型重定向配置加载完成。");

  console.log("正在加载语义模型路由配置...");
  await semanticModelRouter.initialize(
    path.join(__dirname, "SemanticModelRouter.json"),
    DEBUG_MODE
  );
  ...
  server = app.listen(port, "0.0.0.0", () => {
    ...
    if (DEBUG_MODE)
      console.log(
        "[Server] WebSocketServer, PluginManager, and FileFetcherServer have been interconnected."
      );
  });
...
async function gracefulShutdown(exitCode = 0, reason = "signal") {
  if (shutdownPromise) {
    console.log(
      `[Server] gracefulShutdown already in progress. Reusing existing shutdown promise. Current state: ${serverLifecycleState}`
    );
  ...
  console.log(
    `[Server] Initiating graceful shutdown. reason=${reason}, exitCode=${exitCode}`
  );
  console.log(
    `[Server][ShutdownTrace] Phase 0/10 - shutdown requested. activeRequests=${activeRequests.size}`
  );

  forceShutdownTimer = setTimeout(() => {
    console.error(
      "[Server] Graceful shutdown timed out. Forcing process exit."
    );
  ...
  shutdownPromise = (async () => {
    try {
      if (webSocketServer && typeof webSocketServer.beginDrain === "function") {
        console.log(
          `[Server][ShutdownTrace] Phase 1/10 - begin WebSocket drain`
        );
        console.log("[Server] Draining WebSocket upgrade handling...");
        await webSocketServer.beginDrain();
        console.log(
          `[Server][ShutdownTrace] Phase 1/10 - WebSocket drain ready`
        );
      } else {
        console.log(
          `[Server][ShutdownTrace] Phase 1/10 - WebSocket drain skipped`
        );
      }

      console.log(
        `[Server][ShutdownTrace] Phase 2/10 - closing HTTP server listener`
      );
      await closeHttpServerGracefully();
      console.log(
        `[Server][ShutdownTrace] Phase 2/10 - HTTP server listener closed. trackedSockets=${trackedSockets.size}, activeHttpRequests=${activeHttpRequests.size}`
      );

      console.log(
        `[Server][ShutdownTrace] Phase 3/10 - waiting active requests to drain (initial=${activeRequests.size})`
      );
      const drainedNaturally = await waitForActiveRequestsToDrain(30000);
      console.log(
        `[Server][ShutdownTrace] Phase 3/10 - drain wait result=${drainedNaturally}, remaining=${activeRequests.size}`
      );

      if (!drainedNaturally) {
        console.warn(
          `[Server] Active requests did not drain within timeout. Remaining: ${activeRequests.size}`
        );
        console.log(
          `[Server][ShutdownTrace] Phase 4/10 - aborting remaining active requests`
        );
        await abortAllActiveRequests(
          "服务正在重启，当前请求已被服务器安全中止。"
        );
        const drainedAfterAbort = await waitForActiveRequestsToDrain(5000);
        console.log(
          `[Server][ShutdownTrace] Phase 4/10 - abort complete. drainedAfterAbort=${drainedAfterAbort}, remaining=${activeRequests.size}`
        );
      } else {
        console.log(
          `[Server][ShutdownTrace] Phase 4/10 - abort skipped, no remaining active requests`
        );
      }

      serverLifecycleState = SERVER_LIFECYCLE.SHUTTING_DOWN;
      console.log(
        `[Server][ShutdownTrace] Phase 5/10 - lifecycle switched to SHUTTING_DOWN`
      );

      if (taskScheduler) {
        console.log(
          `[Server][ShutdownTrace] Phase 6/10 - taskScheduler.shutdown start`
        );
        taskScheduler.shutdown();
        console.log(
          `[Server][ShutdownTrace] Phase 6/10 - taskScheduler.shutdown done`
        );
      } else {
        console.log(
          `[Server][ShutdownTrace] Phase 6/10 - taskScheduler shutdown skipped`
        );
      }

      if (webSocketServer) {
        console.log(
          `[Server][ShutdownTrace] Phase 7/10 - webSocketServer.shutdown start`
        );
        console.log("[Server] Shutting down WebSocketServer...");
        await Promise.resolve(webSocketServer.shutdown());
        console.log(
          `[Server][ShutdownTrace] Phase 7/10 - webSocketServer.shutdown done`
        );
      } else {
        console.log(
          `[Server][ShutdownTrace] Phase 7/10 - WebSocketServer shutdown skipped`
        );
      }

      if (pluginManager) {
        console.log(
          `[Server][ShutdownTrace] Phase 8/10 - pluginManager.shutdownAllPlugins start`
        );
        await pluginManager.shutdownAllPlugins();
        console.log(
          `[Server][ShutdownTrace] Phase 8/10 - pluginManager.shutdownAllPlugins done`
        );
      } else {
        console.log(
          `[Server][ShutdownTrace] Phase 8/10 - pluginManager shutdown skipped`
        );
      }

      if (tdbKnowledgeManager) {
        console.log(
          `[Server][ShutdownTrace] Phase 9/10 - tdbKnowledgeManager.shutdown start`
        );
        await tdbKnowledgeManager.shutdown();
        console.log(
          `[Server][ShutdownTrace] Phase 9/10 - tdbKnowledgeManager.shutdown done`
        );
      } else {
        console.log(
          `[Server][ShutdownTrace] Phase 9/10 - tdbKnowledgeManager shutdown skipped`
        );
      }

      if (knowledgeBaseManager) {
        console.log(
          `[Server][ShutdownTrace] Phase 9/10 - knowledgeBaseManager.shutdown start`
        );
        await knowledgeBaseManager.shutdown();
        console.log(
          `[Server][ShutdownTrace] Phase 9/10 - knowledgeBaseManager.shutdown done`
        );
      } else {
        console.log(
          `[Server][ShutdownTrace] Phase 9/10 - knowledgeBaseManager shutdown skipped`
        );
      }

      const serverLogWriteStream = logger.getLogWriteStream();
      if (serverLogWriteStream) {
        logger.originalConsoleLog(
          "[Server][ShutdownTrace] Phase 10/10 - closing server log file stream"
        );
        ...
        const logClosePromise = new Promise((resolve) => {
          serverLogWriteStream.end(
            `[${dayjs()
              ...
              )}] Server gracefully shut down. reason=${reason}\n`,
            () => {
              logger.originalConsoleLog("[Server] Server log stream closed.");
              logger.originalConsoleLog(
                "[Server][ShutdownTrace] Phase 10/10 - server log file stream closed"
              );
              resolve();
            }
          );
        ...
        console.log(
          `[Server][ShutdownTrace] Phase 10/10 - log stream close skipped`
        );
      }
    } finally {
  ...
  if (serverLogWriteStream && !serverLogWriteStream.destroyed) {
    try {
      serverLogWriteStream.write(
        `[${dayjs()
          .tz(DEFAULT_TIMEZONE)
          .format("YYYY-MM-DD HH:mm:ss Z")}] [CRITICAL] Uncaught Exception: ${
          error.message
        }\n${error.stack}\n`
      );
    } catch (e) {
  ...
  if (serverLogWriteStream && !serverLogWriteStream.destroyed) {
    try {
      fsSync.appendFileSync(
        currentServerLogPath,
        `[${dayjs()
          .tz(DEFAULT_TIMEZONE)
          .format("YYYY-MM-DD HH:mm:ss Z")}] Server exited with code ${code}.\n`
      );
      serverLogWriteStream.end();
    } catch (e) {
      logger.originalConsoleError(
        "[Server] Error during final log write on exit:",
        e.message
      );
    }
  }
