...
async function writeDebugLog(filenamePrefix, data) {
  if (DEBUG_MODE) {
    ...
    const timestamp = `${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, '0')}${now
      ...
      .getMinutes()
      .toString()
      .padStart(2, '0')}${now.getSeconds().toString().padStart(2, '0')}_${now
      .getMilliseconds()
      .toString()
      .padStart(3, '0')}`;
...
async function ensureRequiredDirectories() {
  const requiredDirectories = [
    ...
    path.join(__dirname, 'TVStxt'),
    path.join(__dirname, 'VCPTimedContacts'),
    path.join(__dirname, 'VCPAsyncResults'),
    path.join(__dirname, 'VectorStore'),
    path.join(__dirname, 'Plugin', 'VCPLog', 'log'),
    path.join(__dirname, 'Plugin', 'EmojiListGenerator', 'generated_lists'),
    path.join(__dirname, 'DebugLog'),
  ];

  for (const dir of requiredDirectories) {
    try {
      await fs.mkdir(dir, { recursive: true });
      console.log(`[DirectoryInit] 已确保目录存在: ${path.relative(__dirname, dir)}`);
    } catch (error) {
      console.error(`[DirectoryInit] 创建目录失败 ${dir}:`, error.message);
    }
  }
  console.log('[DirectoryInit] 关键目录初始化完成。');
}
  ...
  if (isAdminPath) {
    ...
    if (!credentials || credentials.name !== ADMIN_USERNAME || credentials.pass !== ADMIN_PASSWORD) {
      ...
      if (
        ...
        return res
          .status(401)
          .send('<h1>401 Unauthorized</h1><p>Authentication required to access the Admin Panel.</p>');
      }
    }
...
app.post('/v1/schedule_task', async (req, res) => {
  ...
  if (!schedule_time || !task_id || !tool_call || !tool_call.tool_name || !tool_call.arguments) {
    return res
      .status(400)
      .json({ status: 'error', error: "请求无效，缺少 'schedule_time', 'task_id', 或有效的 'tool_call' 对象。" });
  }
  ...
  if (fullAiResponseTextForDiary.trim()) {
    ...
    if (match && match[1]) {
      ...
      if (maidName && dateString && contentText) {
        ...
        try {
          if (DEBUG_MODE)
            console.log('[handleDiaryFromAIResponse] Calling DailyNoteWrite plugin with payload:', diaryPayload);
          ...
          if (pluginResult && pluginResult.status === 'success' && pluginResult.message) {
            ...
            const notification = {
              ...
              data: {
                ...
                filePath: filePath,
                status: 'success',
                message: `日记 '${filePath || '未知路径'}' 已为 '${diaryPayload.maidName}' (${
                  diaryPayload.dateString
                }) 创建成功。`,
              },
            };
            webSocketServer.broadcast(notification, 'VCPLog');
            if (DEBUG_MODE)
              console.log('[handleDiaryFromAIResponse] Broadcasted daily_note_created notification:', notification);
        ...
        console.error('[handleDiaryFromAIResponse] Could not extract Maid, Date, or Content from daily note block:', {
          maidName,
          dateString,
...
async function initialize() {
  ...
  try {
    ...
    if (txtFiles.length === 0) {
      if (DEBUG_MODE)
        console.warn(`[initialize] Warning: No .txt files found in emoji list source directory: ${emojiListSourceDir}`);
    } else {
      if (DEBUG_MODE)
        console.log(`[initialize] Found ${txtFiles.length} emoji list files in ${emojiListSourceDir}. Loading...`);
      await Promise.all(
...
server = app.listen(port, async () => {
  ...
  // 首先确保所有必需的目录存在
  console.log('[Server] 正在初始化必需的目录结构...');
  ...
  if (DEBUG_MODE)
    console.log('[Server] WebSocketServer, PluginManager, and FileFetcherServer have been interconnected.');
  ...
  if (serverLogWriteStream && !serverLogWriteStream.destroyed) {
    try {
      fsSync.appendFileSync(
        ...
        `[${new Date().toLocaleString()}] Server exited with code ${code}.\n`,
      );
