...
- **未来的基石**: 这一坚实的底层架构，为未来 `RagTagMemo` 系统的进一步深入优化和功能拓展打下了不可撼动的基石，确保了 VCP 记忆系统长期、高效、稳定地进化。
	...
	*   **Rust驱动的内存安全与极致性能**: 核心逻辑由Rust构建，引入了原创的语义级Chunk去重函数，杜绝内存安全问题，确保系统长期稳定并完全释放硬件性能。
	...
	-   **纯文本 BM25 召回后缀**：
	    ...
	    -   `::BM25` 与 `::BM25+` 均可和 `::LastN` 配合限定候选范围，例如 `{{小雨日记本::BM25::Last30}}` 或 `{{小雨日记本::BM25+::Last50}}`；如果只写 `::BM25` / `::BM25+` 而不写 `::LastN`，默认只在最新 10 篇日记中检索。
	    -   如果 BM25 没有命中、查询词被净化/屏蔽后为空，或候选文件没有可匹配文本，系统会自动回退为 `::LastN` 的最新文件召回，不会报错。
	...
	-   **适用场景**：当您需要角色完整回顾其所有记忆，或者需要对整个日记进行全面总结时；若只希望在零向量化成本下按标签或正文关键词快速注入少量相关日记，则推荐使用 `::BM25` / `::BM25+` 搭配 `::LastN`；若希望 Agent 每次都获得不同的候选内容（例如随机 BGM、随机播放列表、随机灵感卡片），则推荐使用 `::RandomN`。
	-   **注意**：`{{}}` 纯文本模式不触发向量化、RAG、TagMemo、Rerank 或向量库检索；`::RandomN` 只是本地文件随机抽样与读取；`::BM25` / `::BM25+` 只是本地文件读取、jieba 分词与 BM25 排序。
	...
	-   **最新文件召回后缀**：`<<角色日记本::Last>>` 可在相似度阈值通过后，只全文召回该日记本文件夹中**文件创建/编辑时间最新**的日记文件，默认返回最新 10 个；也支持 `<<角色日记本::Last5>>`、`<<角色日记本::Last20>>` 等写法。该后缀只在 `<<>>` 与 `{{}}` 两种全量召回模式中生效，不影响 `[[]]` / `《《》》` 的 RAG 片段检索逻辑；如果请求数量超过实际文件数，会返回全部可用文件并在后端记录 warn，不会报错。
	...
	**Base64Memo** 为日记本检索引入了跨模态感知能力。它允许系统自动从召回的日记正文中提取多媒体附件，并将其以 Base64 格式注入到当前对话中。
	...
	-   **修饰符兼容建议**：
	    -   推荐组合：`::Rerank`、`::Rerank+`、`::Truncate{阈值}`、`:K倍率`，用于提高冷知识库召回精度。
	    -   门控语法 `《《...知识库》》` 适合长期常驻系统提示词；直接语法 `[[...知识库]]` 适合明确知道当前任务需要该库时使用。
	    -   如果知识库规模较大，建议配合较小 K 倍率与较高 `::Truncate` 阈值，避免一次性注入过多片段。
	-   **配置与管理文件**：
