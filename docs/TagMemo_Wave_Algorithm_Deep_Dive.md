...
正曲线分在本批贡献候选中归一化为 \(\widehat{Geo}(c)\)。

实际混合权重不是固定 \(\alpha\)，而是：

\[
\alpha_c=\alpha\cdot Confidence(c)
\]
...
\alpha_c
\left(
\widehat{Geo}(c)-score_{v9}(c)
\right)
\]

这里的基线是 V9.1 增强向量分数，不是原始 KNN 分数。
...
因此守卫候选不会被统一乘低；它仍可能因为其他候选被校正而相对前移或后移。

`alpha` 只控制固定地形判断的修正幅度，不改变候选曲线分和置信度。候选名次只有在分数轨迹跨过其他候选时才变化。
...
旧配置中的 `minGeoCoverageRatio=0.20` 来自直接交集模型。生产实测中曾出现 967 个候选里已有 162 个贡献者、覆盖率 16.75%，却因未达到 20% 而整批回退。对稀疏曲线模型，这种要求过严。

当前生产配置将其调整为：

```json
"minGeoCoverageRatio": 0.10
```

其含义是：允许少量高质量曲线参与校准，同时仍要求本批至少有一定规模的有效结构证据。
...
| 参数 | 含义 |
|:--|:--|
| `alpha` | 候选置信度之外的全局最大修正强度 |
| `minGeoSamples` | 强/精确接触的证据饱和尺度 |
| `fallbackToKnnOnLowTrust` | 是否启用低可信批次回退 |
| `minFieldTags` | 查询场最少正能量 Tag |
| `minFieldEntropy` | 查询场最低归一化熵 |
| `minGeoCoverageRatio` | 有贡献候选最低比例 |
| `minMaxGeoScore` | 全批最高曲线分最低要求 |
| `minGeoScoreSpread` | 多贡献候选最低曲线分差 |
