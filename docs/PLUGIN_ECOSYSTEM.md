...
- `{{VCPFileToolBox}}`、`{{VCPSearchToolBox}}`、`{{VCPContactToolBox}}`、`{{VCPMemoToolBox}}`、`{{VCPMediaToolBox}}` 由 `toolbox_map.json` 和 `modules/toolboxManager.js` 提供，适合放置核心工具、安全偏好和稳定调用范式。
- `{{VCPDynamicTools}}` 由 `modules/dynamicToolRegistry.js` 提供，适合发现长尾工具、自动感知分布式工具在线状态、减少全量工具说明注入。
- 两者只影响提示词暴露，不改变真正的工具执行路径；真正执行仍由 `PluginManager.processToolCall()`、分布式 WebSocket 路由和 `ToolApprovalManager` 控制。
...
`dynamic_tool_catalog.json` 与 `dynamic_tool_categories.json` 是运行时缓存，不建议手工编辑。旧的“工具列表配置编辑器”也使用 `ToolConfigs/*.json`，但它的用途是手动选择工具并导出静态 `TVStxt/*.txt`，不是 DynamicToolBridge 的配置入口。后台路由应过滤 `dynamic_tool_*` 保留文件，避免误删或误改动态工具缓存。
