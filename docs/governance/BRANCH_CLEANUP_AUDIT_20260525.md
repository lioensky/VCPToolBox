...
| 类别 | 数量 | 操作建议 |
| --- | ---: | --- |
| 占用 worktree 待处理 | 7 | 先检查对应目录状态，再决定保留、合并、归档或删除 worktree |
| 未并入需复核（未占用 worktree） | 14 | 逐分支做 `git log main..branch` / `git diff main...branch` 审查 |
| 已并入可清理（未占用 worktree） | 2 | Package D 已删除 46 个；剩余 2 个因 upstream 保护规则保留 |
...
- 执行包 D 剩余 2 个已并入且未占用 worktree 的本地分支未删除，原因是 upstream 指向 `origin/main` 且未并入该 upstream。
- 除执行包 B 和 C2-safe 外，尚未删除其他 worktree。
- C2-safe 对应的 2 个本地分支仍保留，因普通 `git branch -d` 显示拓扑未 fully merged。
- 未 push，未修改远端。
...
| 复核未并入分支 | 第 3 节、第 3.1 节、第 5.4 节记录未占用未并入分支；C2-safe 执行后当前为 14 个未占用未并入分支、5 个占用未并入分支 | 已完成只读分组 |
| 复核剩余 worktree | 第 2 节和占用 worktree 复核补充记录当前 7 个 worktree；其中 dirty、高风险和 patch-equivalent/superseded 均已分类 | 已完成只读分组 |
...
| 禁止 push、删除分支、删除 worktree、修改远端、force、reset/clean、触碰真实密钥 | Package D 分支删除和 Package C2-safe worktree 移除均已获当前回合明确批准；未 push、未修改远端、未 force、未 reset/clean，未触碰 `.env` / `config.env`，未记录真实密钥值 | 已遵守当前阶段 |
| 任何清理、删除、远端同步或生产线动作必须单独明确批准 | Package D 和 Package C2-safe 已获得当前回合批准并继续执行；Package E、D2-upstream-blocked、C2-branch-blocked 和远端同步仍记录为待批准 | D2-safe 已执行；C2-safe worktree 已移除但分支保留；其余等待单独批准 |

完成度结论：

- 本地审计和文档覆盖已经达到当前 goal 的审查记录要求。
- 治理执行仍未完成，因为 D2-upstream-blocked、C2-branch-blocked、Package E、真实未吸收对照线拆分、`origin/main` 拓扑闭合或远端同步都需要后续单独明确批准或复核。
- 因此当前不能把整个治理 goal 标记为完成；下一步只能在获得具体批准后处理 D2-upstream-blocked、C2-branch-blocked、执行 Package E，或进入某条未吸收对照线的专项小任务。
...
3. Package C：C2-safe 的 2 个 worktree 已移除但分支保留；剩余 Package C 对象涉及 worktree 移除或受保护工作环境，必须逐目录确认路径和 clean 状态；`integration/main-absorb-prod-stable-upstream-20260525` 不纳入清理。
...
- 未并入且占用 worktree：5 个，均已在第 2 节和占用 worktree 复核补充中记录。
- 未并入且未占用 worktree：14 个，其中 6 个 Governance Patch 1B 分支为执行包 E 候选，2 个为 C2-safe worktree 已移除但分支删除被拒绝的历史分支，其余 6 个为真实未吸收或高风险混合对照线。
- 已并入且未占用 worktree：2 个，为执行包 D upstream-blocked 剩余候选。
- `prod/stable` 不在任何清理候选中；即使它已被 `main` 包含，也仍是永久保护稳定生产线。

执行状态：Package B 已执行；Package D 已获批并继续执行，累计删除 46 个本地分支；Package C2-safe 已获批并移除 2 个 clean worktree，但对应本地分支因普通删除被拒绝而保留；Package A1/A2/A3、占用 worktree、未占用未并入分支均已完成只读复核。Package E、C2-branch-blocked 和 D2-upstream-blocked 剩余项仍只是候选或 preflight 记录，任何继续清理都需要后续单独明确批准。
...
- Package D 已继续执行；D2-safe 已删除，D2-upstream-blocked 保留。Package C2-safe 已获批并移除 2 个 worktree，但对应本地分支保留。Package E 已完成可执行性复核，结论是不适合作为普通 `git branch -d` 清理包。

仍需单独明确批准的动作：

- 继续处理 D2-upstream-blocked 剩余 2 个已并入且未占用 worktree 的本地分支。
- 决定 Package E 的 6 个 patch-equivalent 本地分支是继续保留，还是另行授权高风险本地强删除。
- 处理 Package C 中剩余的 clean worktree 分支，以及决定 C2-safe 遗留本地分支是保留还是另行批准高风险本地强删除。
...
| D2-upstream-blocked | `lane8/upstream-intake-20260425`、`staging/vcptoolbox-custom-integration-20260425` | 均为 `main` 祖先且未占用 worktree，但 upstream 指向 `origin/main`，未并入该 upstream | 继续保留，除非单独批准修改本地 upstream 配置后再尝试温和删除；仍不得 `git branch -D` |
...
| C2-worktree-cleared | `codex/vcptoolbox-channelhub-core-20260425`、`codex/vcptoolbox-dingtalk-adapters-20260425` | worktree 已按批准移除；分支仍为 `cherry_plus=0 / cherry_minus=1`，但 `git branch -d` 因拓扑未 fully merged 拒绝删除 | 继续保留，除非单独批准高风险本地强删除；仍不得自动 `git branch -D` |
| C-worktree-clean | `codex/vcptoolbox-memory-rag-governance-20260425` | worktree 干净，`cherry_plus=0 / cherry_minus=1`，但仍占用 worktree，且 upstream 指向 `origin/main` | 单独批准后才可先移除对应 worktree，再尝试 `git branch -d`；若被 upstream 保护拦截即停 |
...
- 当前工作区：`A:/VCP/VCPToolBox-staging-custom-integration`。
- 当前分支：`main`。
- D2-safe 执行前 HEAD：`28d9c71`。
- 当前工作树：干净。
- 本地分支数量：23。
- worktree 数量：7。
- 已并入且未占用 worktree 的剩余本地候选：2；不包含 `main`，不包含 `prod/stable`。
- `git branch --no-merged main`：19 个，其中 5 个占用 worktree，14 个未占用 worktree。
...
2. C2-safe：已获批并移除 2 个干净 worktree；对应本地分支删除被 `git branch -d` 拒绝，继续保留。
...
- 执行包 C2-safe 中的 `A:/VCP/VCPToolBox-channelhub-core` 与 `A:/VCP/VCPToolBox-dingtalk-adapters` 已在获批后移除；对应本地分支因普通删除被拒绝而保留。
...
| 分支组 | 当前证据 | 治理结论 |
| --- | --- | --- |
| `codex/vcptoolbox-channelhub-core-20260425`、`codex/vcptoolbox-dingtalk-adapters-20260425` | C2-safe 已移除对应 worktree；`git cherry -v main` 均为 `cherry_plus=0 / cherry_minus=1`；提交已 patch-equivalent | 不需要吸收。本地分支因普通删除被拒绝而保留；如后续清理，需要单独批准高风险本地强删除 |
| `codex/vcptoolbox-memory-rag-governance-20260425` | worktree 仍干净且仍占用；`git cherry -v main` 为 `cherry_plus=0 / cherry_minus=1`；upstream 指向 `origin/main` | 不需要吸收。若后续清理，需单独批准移除 worktree；若 `git branch -d` 被 upstream 保护拦截，必须停止 |
...
结论：占用 worktree 的未并入分支已按当前 `main` 补充复核。C2-safe 的 2 个 worktree 已移除但分支保留；`codex/vcptoolbox-memory-rag-governance-20260425` 仍是 patch-equivalent 且占用 worktree；`lane10`、`codex/photo-studio-baserow-provider-batch`、`feature/latest-updates` 属于真实未吸收或高风险混合对照线；`integration/main-absorb-prod-stable-upstream-20260525` 继续作为稳定线工作环境保留。
...
结论：原 12 个未占用未并入分支已按组复核。C2-safe 执行后新增 2 个 worktree 已移除但分支保留的 patch-equivalent 历史分支；当前未占用未并入分支共 14 个。只有 6 个 Governance Patch 1B 分支是 patch-equivalent 清理候选；2 个 C2-safe 遗留分支需单独批准高风险本地强删除或继续保留；其余 6 个属于真实未吸收或高风险混合对照线，不进入清理候选。
...
| C. patch-equivalent / superseded worktree | `codex/vcptoolbox-channelhub-core-20260425`、`codex/vcptoolbox-dingtalk-adapters-20260425`、`codex/vcptoolbox-memory-rag-governance-20260425`、`integration/main-absorb-prod-stable-upstream-20260525` | C2-safe 的 2 个 worktree 已移除但本地分支保留；memory-rag 和 prod-stable 工作环境仍占用 worktree | 剩余清理必须逐项批准；不得因 branch-delete 失败自动强删 |
| D. 未占用且已并入本地分支 | 第 4 节为执行前 48 个分支快照；当前剩余 2 个 upstream-blocked 见第 5.3.3 节 | D2-safe 已执行，D2-upstream-blocked 保留 | 继续前需单独批准修改本地 upstream 配置或更高风险删除策略；不得自动升级为 `git branch -D` |
...
结论：Package D 的温和可执行部分已完成。剩余 2 个 D2-upstream-blocked 分支继续保留，除非后续单独明确批准修改本地 upstream 配置后再尝试温和删除，或单独批准更高风险的本地强删除策略。
