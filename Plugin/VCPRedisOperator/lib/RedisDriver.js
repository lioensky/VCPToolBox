...
class RedisDriver {
  constructor(env) {
    ...
    this.config = {
      ...
      lazyConnect: true,
      pluginEnabled: env.VCPRedisOperator_ENABLE,
      retryStrategy: (times) => {
        if (
          !env.VCPRedisOperator_ENABLE ||
          env.VCPRedisOperator_ENABLE === "false"
        ) {
          return null; // 停止重试
        }
  ...
  async connect() {
    ...
    this.client.on("error", (err) => {
      if (
        ...
        (!this.config.pluginEnabled || this.config.pluginEnabled === "false")
      ) {
        // 静默处理，避免未开启插件时刷屏
        return;
      }
    ...
    try {
      ...
      if (this.debug)
        console.error(
          "[VCPRedisOperator] Redis initial connection failed:",
          err.message
        );
    }
  }
  ...
  async execute(command, key, args = []) {
    ...
    if (this.debug) {
      console.log(
        `[VCPRedisOperator] CMD: ${command} ${allArgs
    ...
    if (this.debug) {
      console.log(
        `[VCPRedisOperator] RESULT (${duration}ms): ${JSON.stringify(raw)}`
      );
    }
