...
const DOC_PATH = path.resolve(TVSTXT_DIR, 'ContactToolBox.txt');
    ...
    try {
        ...
        const fallback = {
            ...
            fold_blocks: [
                {
                    threshold: 0.0,
                    content: `[ToolBoxFold] 无法读取配置文件 \`ContactToolBox.txt\`。请在 VCPToolBox/TVStxt 目录创建该文件。错误信息: ${error.message}`
                }
            ]
        };
