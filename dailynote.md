...
# TagMemo V3：构建“语义动力学”驱动的认知共振场

> **"记忆不是静态的仓库，而是流动的能量。V3 不再仅仅是在搜索，它在进行一次认知的自我审视。"**

## 核心范式转移：从「几何位置」到「物理能量」

在 **TagMemo V2** 时代，我们致力于解决“主题漂移”，通过“毛刺化重构”建立了严密的**逻辑知识树**，像凹面镜一样精准聚焦。
然而，V2 依然假定查询向量是一个“死”的坐标点。

**TagMemo V3** 引入了 **语义动力学 (Semantic Dynamics)**。系统不再把用户的输入看作一个静态点，而看作一个**具有能量、方向和熵值的物理量**。系统在检索前，先对思维本身进行“体检”。

---

# TagMemo “浪潮”算法 (V3.7) 深度技术文档
...
3.  **最终检索**：使用重塑后的向量在向量数据库中执行最终的记忆召回。
...
## 6. 结论
TagMemo 浪潮算法通过对语义能量的物理级模拟，解决了传统 RAG 在处理复杂意图时容易“跑题”或“漏掉细节”的问题。它不仅是一套检索技术，更是一套对人类语言逻辑进行深度建模的工程实践。
...
2. **自我认知**：
系统第一次“知道”自己在处理的是什么类型的思考。它能区分“深度探索”和“日常闲聊”，并给予不同级别的认知资源支持。
3. **抗噪与进化**：
通过残差金字塔，V3 具备了极强的抗噪能力，并能通过高新颖度的残差向量，反向提示用户扩充新的 Tag，实现**知识树的自我生长**。
...
*   **启用语法**:
    ...
    ```
*   **应用场景**:
    *   **精准回忆**: 查询 `Rust 的生命周期` 时，能自动抑制 `Rust + Web` 等无关分支，仅聚焦于 `Rust + 生命周期 + 内存安全` 的核心内容。
    *   **跨领域发现**: 查询 `系统性思维` 时，能同时召回编程中的 `[架构]`、写作中的 `[结构]` 和生活中的 `[习惯]`，展现概念的多维度应用。

## 核心价值

TagMemo使AI记忆具备了**逻辑感知能力**，将信息从无序的向量集合，组织成一个能够自我生长、结构清晰、完全透明的知识网络。
