...
export interface OneRingLoadResult {
  success: boolean
  loadedCount: number
  elapsedMs: number
  message: string
}
export interface OneRingPostBlock {
  role: string
  text: string
  senderName?: string
...
export interface OneRingDiffInput {
  dbPath: string
  agentName: string
  frontendSource: string
  postBlocks: Array<OneRingPostBlock>
  threshold: number
  limit: number
}
export interface OneRingEditedBlock {
  postIndex: number
  dbId: number
  oldContent: string
  newText: string
...
export interface OneRingNewBlock {
  postIndex: number
  role: string
  text: string
  senderName?: string
...
export interface OneRingDiffResult {
  matchedCount: number
  unknownCount: number
  editedBlocks: Array<OneRingEditedBlock>
  newBlocks: Array<OneRingNewBlock>
  reliable: boolean
  elapsedMs: number
  phaseSummary: string
}
export interface OneRingMessageInput {
  dbPath: string
  agentName: string
  role: string
  senderName?: string
...
export interface OneRingUpdateInput {
  dbPath: string
  agentName: string
  id: number
  content: string
}
export interface OneRingWriteResult {
  success: boolean
  id: number
  elapsedMs: number
  message: string
}
export interface OneRingEngineStats {
  agents: number
  records: number
  frontends: number
}
...
export declare class OneRingEngine {
  constructor()
  loadAgent(dbPath: string, agentName: string, maxRecords?: number | undefined | null): OneRingLoadResult
  diffFrontendContext(input: OneRingDiffInput): OneRingDiffResult
  insertMessage(input: OneRingMessageInput): OneRingWriteResult
  updateMessageById(input: OneRingUpdateInput): OneRingWriteResult
  stats(): OneRingEngineStats
}
/** 核心索引结构 (无状态，只存向量) */
