...
class KnowledgeBaseManager {
  constructor(config = {}) {
    this.config = {
      ...
      storePath:
        ...
        path.join(__dirname, "VectorStore"),
      ...
      batchWindow:
        parseInt(process.env.KNOWLEDGEBASE_BATCH_WINDOW_MS, 10) || 2000,
      maxBatchSize:
        parseInt(process.env.KNOWLEDGEBASE_MAX_BATCH_SIZE, 10) || 50,
      indexSaveDelay:
        parseInt(process.env.KNOWLEDGEBASE_INDEX_SAVE_DELAY, 10) || 120000,
      tagIndexSaveDelay:
        parseInt(process.env.KNOWLEDGEBASE_TAG_INDEX_SAVE_DELAY, 10) || 300000,
      deleteBatchWindow:
        parseInt(process.env.KNOWLEDGEBASE_DELETE_BATCH_WINDOW_MS, 10) || 1000,
      maxDeleteBatchSize:
        parseInt(process.env.KNOWLEDGEBASE_MAX_DELETE_BATCH_SIZE, 10) || 2000,
      deleteRebuildThreshold:
        parseInt(process.env.KNOWLEDGEBASE_DELETE_REBUILD_THRESHOLD, 10) ||
        5000,
      migrationCacheTtlMs:
        parseInt(process.env.KNOWLEDGEBASE_MIGRATION_CACHE_TTL_MS, 10) ||
        2 * 60 * 1000,
      // 🛡️ Rust 派生表写入租约：避免 rusqlite 与 better-sqlite3 双写 WAL 竞态
      rustWriteLeaseGraceMs:
        parseInt(process.env.KNOWLEDGEBASE_RUST_WRITE_LEASE_GRACE_MS, 10) ||
        30000,
      rustWriteLeaseCooldownMs:
        parseInt(process.env.KNOWLEDGEBASE_RUST_WRITE_LEASE_COOLDOWN_MS, 10) ||
        10000,
      rustWriteLeaseCheckpointBeforeGrant:
        (
          process.env.KNOWLEDGEBASE_RUST_WRITE_LEASE_CHECKPOINT_BEFORE_GRANT ||
          "true"
        ).toLowerCase() === "true",
      rustWriteLeaseRetryMs:
        parseInt(process.env.KNOWLEDGEBASE_RUST_WRITE_LEASE_RETRY_MS, 10) ||
        1000,
      rustWriteLeaseTtlMs:
        parseInt(process.env.KNOWLEDGEBASE_RUST_WRITE_LEASE_TTL_MS, 10) ||
        10 * 60 * 1000,
      rustWriteLeaseMaxWaitMs:
        parseInt(process.env.KNOWLEDGEBASE_RUST_WRITE_LEASE_MAX_WAIT_MS, 10) ||
        30 * 60 * 1000,
      rustWriteLeasePendingThreshold:
        parseInt(
          process.env.KNOWLEDGEBASE_RUST_WRITE_LEASE_PENDING_THRESHOLD,
          10
        ) || 0,
      derivedStartupCooldownMs:
        parseInt(process.env.KNOWLEDGEBASE_DERIVED_STARTUP_COOLDOWN_MS, 10) ||
        5 * 60 * 1000,
      // 🌟 索引空闲自动卸载：默认 2 小时未使用则从内存中卸载
      indexIdleTTL:
        parseInt(process.env.KNOWLEDGEBASE_INDEX_IDLE_TTL_MS, 10) ||
        2 * 60 * 60 * 1000,
      indexIdleSweepInterval:
        parseInt(process.env.KNOWLEDGEBASE_INDEX_IDLE_SWEEP_MS, 10) ||
        10 * 60 * 1000,
      idleSweepLogTick:
        (
          process.env.KNOWLEDGEBASE_IDLE_SWEEP_LOG_TICK || "false"
        ).toLowerCase() === "true",

      ignoreFolders: (process.env.IGNORE_FOLDERS || "VCP论坛")
        .split(",")
        .map((f) => f.trim())
        .filter(Boolean),
      ignorePrefixes: (
        process.env.IGNORE_PREFIXES ||
        process.env.IGNORE_PREFIX ||
        "已整理"
      )
        .split(",")
        .map((p) => p.trim())
      ...
      tagBlacklist: new Set(
        (process.env.TAG_BLACKLIST || "")
          .split(",")
          .map((t) => t.trim())
          .filter(Boolean)
      ),
      tagBlacklistSuper: (process.env.TAG_BLACKLIST_SUPER || "")
        .split(",")
        .map((t) => t.trim())
        .filter(Boolean),
      ...
      fullScanOnStartup:
        (
          process.env.KNOWLEDGEBASE_FULL_SCAN_ON_STARTUP || "true"
        ).toLowerCase() === "true",
      // 语言置信度补偿配置
      langConfidenceEnabled:
        (process.env.LANG_CONFIDENCE_GATING_ENABLED || "true").toLowerCase() ===
        "true",
      ...
      persistTagIndex:
        (
          process.env.KNOWLEDGEBASE_PERSIST_TAG_INDEX || "false"
        ).toLowerCase() === "true",
      // 🌟 强制开启持久化的文件夹白名单 (支持中英文逗号)
      persistFolders: new Set(
        (process.env.KNOWLEDGEBASE_PERSIST_FOLDERS || "")
          .split(/[,，]/)
          .map((f) => f.trim())
          .filter(Boolean)
      ),
  ...
  async initialize() {
    ...
    console.log(
      `[KnowledgeBase] Initializing Multi-Index System (Dim: ${this.config.dimension})...`
    );
    ...
    if (shouldPersistTags && fsSync.existsSync(tagIdxPath)) {
      try {
        ...
        console.warn(
          `[KnowledgeBase] ⚠️ Failed to load tag index from disk: ${e.message}. Rebuilding...`
        );
      }
    }
    ...
    if (!indexReady) {
      console.log(
        "[KnowledgeBase] 🚀 Building Global Tag Index from SQLite..."
      );
      ...
      try {
        ...
        console.log(
          `[KnowledgeBase] ✅ Global Tag Index ready. ${count} vectors indexed.`
        );
        // 如果开启了持久化但文件不存在，则保存一次
        if (shouldPersistTags) this._saveIndexToDisk("global_tags");
      } catch (e) {
        console.error(
          `[KnowledgeBase] ❌ Global Tag Index recovery failed: ${e.message}`
        );
      }
    }

    // 2. 预热日记本名称向量缓存（使用 setImmediate 避免阻塞主事件循环）
    setImmediate(() => {
    ...
    // 初始化浪潮引擎
    this.tagMemoEngine = new TagMemoEngine(
      this.db,
      this.tagIndex,
      this.config,
      this.ragParams,
    ...
    if (
      this.tagMemoEngine &&
      typeof this.tagMemoEngine.schedulePostStartupDerivedRefresh === "function"
    ) {
      this.tagMemoEngine.schedulePostStartupDerivedRefresh(
  ...
  async loadRagParams() {
    ...
    try {
      ...
      if (this.tagMemoEngine)
        this.tagMemoEngine.updateRagParams(this.ragParams);
    } catch (e) {
  ...
  _startRagParamsWatcher() {
    ...
    this.ragParamsWatcher.on("change", async () => {
      console.log(
        "[KnowledgeBase] 🔄 检测到 rag_params.json 变更，正在重新加载..."
      );
      await this.loadRagParams();
    });
  }
  ...
  _openDatabaseWithRecovery(dbPath) {
    ...
    try {
      ...
      console.error(
        "[KnowledgeBase] ❌ SQLite database corruption detected during startup."
      );
      ...
      const backupBase = this._quarantineSqliteDatabase(
        dbPath,
        "startup-corrupt"
      );
      console.warn(
        `[KnowledgeBase] 🧯 Corrupt SQLite database quarantined as "${path.basename(
          backupBase
        )}*". ` +
  ...
  checkpointAndAssertDatabaseHealthy(reason = "manual-checkpoint") {
    ...
    try {
      ...
      if (!this._isSqliteCorruptionError(e)) {
        console.error(
          `[KnowledgeBase] 🚨 SQLite checkpoint/quick_check failed after ${reason}: ${
            e.message || e
          }`
        );
        return false;
      ...
      console.warn(
        `[KnowledgeBase] 🩺 SQLite checkpoint/quick_check reported suspect state after ${reason}: ${
          e.message || e
        }`
      );
  ...
  _rebindDatabaseConnection(db) {
    ...
    if (this.tagMemoEngine) {
      ...
      if (this.tagMemoEngine.residualPyramid)
        this.tagMemoEngine.residualPyramid.db = db;
    }

    if (this.resultDeduplicator) {
      this.resultDeduplicator.db = db;
      if (this.resultDeduplicator.epa) this.resultDeduplicator.epa.db = db;
      if (this.resultDeduplicator.residualCalculator)
        this.resultDeduplicator.residualCalculator.db = db;
    }
  }
  ...
  _recoverSuspectDatabaseConnection(reason, firstError) {
    ...
    try {
      console.warn(
        `[KnowledgeBase] 🩺 SQLite suspect state after ${reason}; reopening connection for second-stage verification...`
      );
      try {
        oldDb?.close();
      } catch (closeErr) {
        console.warn(
          `[KnowledgeBase] ⚠️ Failed to close suspect SQLite connection cleanly: ${closeErr.message}`
        );
      }
      ...
      this.databaseCorruptionDetected = false;
      console.warn(
        "[KnowledgeBase] ✅ SQLite suspect verification passed after reopen; treating as transient WAL/SHM view issue."
      );
      return true;
    } catch (secondError) {
      console.error(
        `[KnowledgeBase] 🚨 SQLite second-stage verification failed after ${reason}: ${
          secondError.message || secondError
        }`
      );
      console.error(
        `[KnowledgeBase] First-stage failure was: ${
  ...
  _isSqliteCorruptionError(e) {
    ...
    return (
      ...
      /database disk image is malformed|file is not a database|database corruption|quick_check failed/i.test(
        message
      )
    );
  }
  ...
  _quarantineSqliteDatabase(dbPath, reason = "corrupt") {
    ...
    for (const file of relatedFiles) {
      ...
      try {
        ...
        console.warn(
          `[KnowledgeBase] 🧯 Quarantined "${path.basename(
            file
          )}" -> "${path.basename(target)}"`
        );
      } catch (err) {
        console.error(
          `[KnowledgeBase] ❌ Failed to quarantine "${file}": ${err.message}`
        );
        throw err;
      }
    }
  ...
  async _handleRuntimeSqliteCorruption(error, batchFiles = []) {
    ...
    console.error(
      "[KnowledgeBase] 🚨 SQLite database corruption detected at runtime; batch processing is paused."
    );
    console.error(
      `[KnowledgeBase] Runtime corruption details: ${error?.message || error}`
    );
    console.error(
    ...
    try {
      if (this.watcher) {
        ...
        this.watcher = null;
        console.error(
          "[KnowledgeBase] 🛑 File watcher stopped to prevent retry storms against a corrupt SQLite database."
        );
      }
    } catch (watchErr) {
      console.warn(
        `[KnowledgeBase] ⚠️ Failed to stop watcher after SQLite corruption: ${watchErr.message}`
      );
    }
  }
  ...
  _startEventLoopWatchdog() {
    ...
    const intervalMs =
      parseInt(process.env.KNOWLEDGEBASE_EVENT_LOOP_WATCHDOG_MS, 10) || 5000;
    const warnLagMs =
      parseInt(process.env.KNOWLEDGEBASE_EVENT_LOOP_WATCHDOG_WARN_LAG_MS, 10) ||
      2000;
      ...
      if (lag >= warnLagMs) {
        console.warn(
          `[KnowledgeBase] 🧯 Event loop lag detected: ${lag}ms. ` +
            ...
            `rustLease=${this.rustWriteLease?.owner || "none"}, loadedIndices=${
              this.diaryIndices.size
            }, ` +
    ...
    console.log(
      `[KnowledgeBase] 🧯 Event loop watchdog started (interval=${intervalMs}ms, warnLag=${warnLagMs}ms).`
    );
  }
  ...
  _isRustWriteLeaseExpired(now = Date.now()) {
    return (
      this.rustWriteLease &&
      now - this.rustWriteLease.startedAt >
        (this.rustWriteLease.ttlMs || this.config.rustWriteLeaseTtlMs)
    );
  }
  ...
  _canGrantRustWriteLease(options = {}) {
    ...
    if (this.dbHealthState !== "healthy")
      return { ok: false, reason: `database-${this.dbHealthState}` };
      ...
      if (sinceStartupReady < this.config.derivedStartupCooldownMs) {
        return {
          ...
          reason: `startup-cooldown:${
            this.config.derivedStartupCooldownMs - sinceStartupReady
          }ms`,
        };
      }
    }
    ...
    if (this.rustWriteLease)
      return {
        ok: false,
        reason: `rust-lease-active:${this.rustWriteLease.owner}`,
      };
    ...
    if (this.pendingDeletes.size > 0)
      return {
        ok: false,
        reason: `pending-deletes:${this.pendingDeletes.size}`,
      };

    const threshold =
      options.pendingThreshold ?? this.config.rustWriteLeasePendingThreshold;
    if (threshold >= 0 && this.pendingFiles.size > threshold) {
      return {
        ok: false,
        reason: `pending-files:${this.pendingFiles.size}>${threshold}`,
      };
    }
    ...
    if (
      this.lastRustWriteFinishedAt > 0 &&
      sinceRustWrite < this.config.rustWriteLeaseCooldownMs
    ) {
      return {
        ok: false,
        reason: `rust-write-cooldown:${
          this.config.rustWriteLeaseCooldownMs - sinceRustWrite
        }ms`,
      };
    }
    ...
    while (true) {
      ...
      if (decision.ok) {
        if (this.config.rustWriteLeaseCheckpointBeforeGrant) {
          const healthy = this.checkpointAndAssertDatabaseHealthy(
            `granting Rust lease "${owner}"`
          );
          if (!healthy) {
            console.error(
              `[KnowledgeBase] 🦀🚫 Rust SQLite write lease "${owner}" denied because database health check failed.`
            );
            return null;
          }
        }
        ...
        console.log(
          `[KnowledgeBase] 🦀🔐 Rust SQLite write lease granted to "${owner}".`
        );
        return {
          owner,
  ...
  releaseRustWriteLease(owner) {
    ...
    console.log(
      `[KnowledgeBase] 🦀🔓 Rust SQLite write lease released by "${owner}".`
    );
    ...
    if (!this.databaseCorruptionDetected) {
      if (this.pendingDeletes.size > 0) {
        setTimeout(
          ...
          this.config.rustWriteLeaseCooldownMs
        );
      }
  ...
  _deferBatchForRustLease(type = "batch") {
    ...
    console.log(
      `[KnowledgeBase] 🦀⏸️ Deferring ${type} while Rust SQLite write lease is active (${owner}).`
    );
    setTimeout(() => {
  ...
  _decodeVectorBlob(blob, dim, label = "vector") {
    ...
    if (blob.length !== expectedBytes) {
      console.warn(
        `[KnowledgeBase] ⚠️ Invalid ${label} blob length: expected ${expectedBytes}, got ${blob.length}`
      );
      return null;
    }
  ...
  _cleanupStalePairwiseSimilarityModels() {
    try {
      ...
      const result = this.db
        .prepare("DELETE FROM tag_pair_similarity WHERE model_sig != ?")
        .run(this.tagMemoEngine.modelSig);

      if (result.changes > 0) {
        console.warn(
          `[KnowledgeBase] 🧹 Removed ${result.changes} stale pairwise similarity row(s) from old embedding model signatures.`
        );
      }
    } catch (e) {
      console.warn(
        "[KnowledgeBase] ⚠️ Failed to cleanup stale pairwise similarity model rows:",
        e.message
      );
    }
  }
  ...
  _cleanupDatabaseOrphans() {
    try {
      ...
      const missingFiles = this.db
        .prepare("SELECT id, path, diary_name FROM files")
        .all()
        .filter(
          (row) => !fsSync.existsSync(path.join(this.config.rootPath, row.path))
        );

      missingFiles.forEach((row) => affectedDiaries.add(row.diary_name));

      const orphanChunkCount =
        this.db
          .prepare(
            `
      ...
      if (
        ...
        orphanChunkCount > 0 ||
        affectedDiaries.size > 0
      ) {
        console.warn(
          `[KnowledgeBase] 🧹 Startup cleanup complete. Removed ${missingFiles.length} missing file record(s), ${orphanChunkCount} orphan chunk(s), touched ${affectedDiaries.size} diary index(es).`
        );
      }
    } catch (e) {
      console.error(
        "[KnowledgeBase] ❌ Startup database cleanup failed:",
        e.message || e
      );
    }
  }
  ...
  _deletePersistedDiaryIndex(diaryName) {
    const shouldPersist =
      ...
      this.config.persistFolders.has(diaryName) ||
      diaryName.endsWith("簇");
    ...
    try {
      if (fsSync.existsSync(idxPath)) {
        ...
        console.warn(
          `[KnowledgeBase] 🧹 Removed stale persisted index for diary "${diaryName}". It will be rebuilt from SQLite.`
        );
      }
      if (fsSync.existsSync(tmpPath)) fsSync.unlinkSync(tmpPath);
    } catch (e) {
      console.warn(
        `[KnowledgeBase] ⚠️ Failed to remove stale persisted index for "${diaryName}": ${e.message}`
      );
    }
  }
  ...
  _deleteAllPersistedDiaryIndexes() {
    try {
      ...
      console.warn(
        "[KnowledgeBase] 🧹 Removed all persisted diary indexes because orphan chunks had lost diary ownership metadata."
      );
    } catch (e) {
      console.warn(
        `[KnowledgeBase] ⚠️ Failed to remove all persisted diary indexes: ${e.message}`
      );
    }
  }
  ...
  async _getOrLoadDiaryIndex(diaryName) {
    ...
    const shouldPersist =
      ...
      diaryName.endsWith("簇");
    console.log(
      `[KnowledgeBase] 📂 Loading index for diary: "${diaryName}" (Persist: ${shouldPersist})`
    );
    ...
    let idx;
    if (shouldPersist) {
      idx = await this._loadOrBuildIndex(
        fileName,
        capacity,
        "chunks",
        diaryName
  ...
  async _loadOrBuildIndex(
    fileName,
    capacity,
    tableType,
    filterDiaryName = null
  ) {
    const idxPath = path.join(
      this.config.storePath,
    ...
    try {
      if (fsSync.existsSync(idxPath)) {
        ...
        console.log(
          `[KnowledgeBase] Index file not found for ${fileName}, rebuilding from SQLite when possible.`
        );
        idx = new VexusIndex(this.config.dimension, capacity);
        if (filterDiaryName) {
      ...
      console.error(
        `[KnowledgeBase] Index load error (${fileName}): ${e.message}`
      );
      console.warn(
        `[KnowledgeBase] Rebuilding index ${fileName} from DB as a fallback...`
      );
      idx = new VexusIndex(this.config.dimension, capacity);
      await this._recoverIndexFromDB(idx, tableType, filterDiaryName);
    }
  ...
  async _recoverIndexFromDB(vexusIdx, table, diaryName) {
    console.log(
      `[KnowledgeBase] 🔄 Recovering ${table} (Filter: ${
        diaryName || "None"
      }) via Rust...`
    );
    try {
      ...
      const count = await vexusIdx.recoverFromSqlite(
        dbPath,
        table,
        diaryName || null
      );
...
=======
      ...
      if (diaryName) {
        return await this._searchSpecificIndex(
          diaryName,
          queryVec,
          k,
          tagBoost,
        ...
        return await this._searchAllIndices(
          queryVec,
          k,
          tagBoost,
          coreTags,
  ...
  async _searchSpecificIndex(
    ...
    try {
      ...
      if (searchVecFloat.length !== this.config.dimension) {
        console.error(
          `[KnowledgeBase] Dimension mismatch! Expected ${this.config.dimension}, got ${searchVecFloat.length}`
        );
        return [];
      }
    } catch (err) {
      ...
      return [];
>>>>>>> abbf1d79 (feat(core): implement automatic port downgrade and fix cold start blocking)
    }
    ...
    try {
      ...
      // 🛠️ 修复 2: 详细的错误日志
      console.error(
        `[KnowledgeBase] Vexus search failed for "${diaryName}":`,
        e.message || e
      );
      return [];
    }
...
if (tagBoost > 0 && this.tagMemoEngine) {
        ...
        for (const row of fileTagRows) {
          if (!fileTagNameMap.has(row.file_id))
            fileTagNameMap.set(row.file_id, []);
          fileTagNameMap.get(row.file_id).push(row.name);
        }
        }

        // 将查询级 coreTags 转为 Set（用于交叉匹配）
        const queryCoreTags = new Set(
          (tagInfo.coreTagsMatched || []).map((t) => t.toLowerCase())
        );
        const queryAllTags = new Set(
          (tagInfo.matchedTags || []).map((t) => t.toLowerCase())
        );
        ...
        for (const r of hydratedResults) {
          ...
          r.coreTagsMatched = chunkRealTags.filter((t) =>
            queryCoreTags.has(t.toLowerCase())
          );
        }
      }
    } else {
...
=======
  ...
  async _searchAllIndices(
    ...
    const allDiaries = this.db
      .prepare("SELECT DISTINCT diary_name FROM files")
      .all();
    ...
    const searchPromises = allDiaries.map(async ({ diary_name }) => {
      try {
        ...
        console.error(
          `[KnowledgeBase] Vexus search error in parallel global search (${diary_name}):`,
          e
        );
        return [];
      }
...
/**
  ...
  async getDiaryNameVector(diaryName) {
    ...
    try {
      ...
      console.warn(
        `[KnowledgeBase] DB lookup failed for diary name: ${diaryName}`
      );
    }

    // 3. 缓存未命中，同步等待向量化
    console.warn(
      `[KnowledgeBase] Cache MISS for diary name vector: "${diaryName}". Fetching now...`
    );
...
// 兼容性 API: getVectorByText
    ...
    async getVectorByChunkId(chunkId) {
      ...
      console.log(
        `[KnowledgeBase] Cache MISS for plugin description. Fetching API...`
      );
      ...
      console.error(
        `[KnowledgeBase] Failed to process plugin description vector:`,
        e.message
      );
    }
    return null;
  ...
  async getVectorByText(diaryName, text) {
    const stmt = this.db.prepare(
      "SELECT vector FROM chunks WHERE content = ? LIMIT 1"
    );
    const row = stmt.get(text);
    if (row && row.vector) {
      const decoded = this._decodeVectorBlob(
  ...
  _hasCompleteStoredVectorsForFile(relPath) {
    try {
      ...
      console.warn(
        `[KnowledgeBase] ⚠️ Failed to check stored vectors for "${relPath}": ${e.message}`
      );
      return false;
    }
  }
  ...
  _cleanupExpiredMigrationCache(now = Date.now()) {
    try {
      ...
      if (result.changes > 0) {
        console.log(
          `[KnowledgeBase] 🧹 Cleaned ${result.changes} expired migration cache file tombstone(s).`
        );
      }
    } catch (e) {
      console.warn(
        `[KnowledgeBase] ⚠️ Failed to cleanup migration cache: ${e.message}`
      );
    }
  }
  ...
  _findReusableChunkVectors(doc) {
    try {
      ...
      for (const candidate of candidates) {
        ...
        const vectors = this._decodeReusableChunkRows(
          rows,
          doc.chunks.length,
        ...
        if (vectors) {
          console.log(
            `[KnowledgeBase] ♻️ Reusing ${vectors.length} cached chunk vector(s) for moved/copied file "${doc.relPath}" from live record "${candidate.path}".`
          );
          return vectors;
        }
      }
      ...
      for (const tombstone of tombstones) {
        ...
        if (vectors) {
          ...
          console.log(
            `[KnowledgeBase] ♻️ Reusing ${vectors.length} cached chunk vector(s) for moved file "${doc.relPath}" from recently deleted "${tombstone.old_path}".`
          );
          return vectors;
        }
      }
      ...
      console.warn(
        `[KnowledgeBase] ⚠️ Failed to lookup reusable vectors for "${
          doc?.relPath || "unknown"
        }": ${e.message}`
      );
    }
...
const stmt = this.db.prepare(`
  ...
  _startWatcher() {
    ...
    const scanInitialFiles = () => {
      ...
      const walk = (dir) => {
        ...
        try {
          ...
          console.warn(
            `[KnowledgeBase] Initial scan skipped unreadable directory "${dir}": ${e.message}`
          );
          return;
        }
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            if (
              ...
              this.config.ignorePrefixes.some((prefix) =>
                entry.name.startsWith(prefix)
              ) ||
              this.config.ignoreSuffixes.some((suffix) =>
          ...
          if (
            this.config.ignorePrefixes.some(
              (prefix) =>
            ...
            continue;
          if (
            this.config.ignoreSuffixes.some(
              (suffix) =>
    ...
    const handleFileWithLock = async (filePath) => {
      ...
      try {
        ...
        if (e.code !== "ENOENT")
          console.warn(`[KnowledgeBase] Stability check error:`, e.message);
      }
    };
    ...
    if (
      VexusIndex &&
      VexusIndex.prototype &&
      typeof VexusIndex.prototype.start_watch === "undefined"
    ) {
      ...
      try {
        ...
        if (vexusModule.VexusWatcher) {
          ...
          const handleRustEvent = (...args) => {
            try {
              ...
              if (!jsonPayload) {
                console.warn(
                  "[KnowledgeBase] Ignored Rust watcher callback without string payload:",
                  args
                );
                return;
              }
        ...
        console.warn(
          "[KnowledgeBase] ⚠️ Failed to initialize Rust Watcher, falling back to Chokidar:",
          e.message
        );
      }
    }
    ...
    const handleChokidarFile = (filePath) => {
      ...
      if (
        this.config.ignorePrefixes.some((prefix) =>
          diaryName.startsWith(prefix)
        )
      )
        return;
      if (
        this.config.ignoreSuffixes.some((suffix) => diaryName.endsWith(suffix))
      )
        return;
      const fileName = path.basename(relPath);
      if (
        this.config.ignorePrefixes.some((prefix) => fileName.startsWith(prefix))
      )
        return;
      if (
        this.config.ignoreSuffixes.some((suffix) => fileName.endsWith(suffix))
      )
        return;
    ...
    this.watcher
      .on("add", handleChokidarFile)
      .on("change", handleChokidarFile)
      .on("unlink", (fp) => this._queueDelete(fp));
  ...
  _scheduleDeleteBatch() {
    ...
    this.deleteBatchTimer = setTimeout(
      () => this._flushDeleteBatch(),
      this.config.deleteBatchWindow
    );
  }

  async _flushDeleteBatch() {
    if (
      this.isProcessingDeletes ||
    ...
    const batchFiles = Array.from(this.pendingDeletes).slice(
      0,
      this.config.maxDeleteBatchSize
    );
  ...
  async _flushBatch() {
    ...
    const batchFiles = Array.from(this.pendingFiles).slice(
      0,
    ...
    try {
      ...
      const checkFile = this.db.prepare(
        "SELECT checksum, mtime, size FROM files WHERE path = ?"
      );
        ...
        batchFiles.map(async (filePath) => {
          try {
            ...
            if (
              ...
              row.size === stats.size &&
              this._hasCompleteStoredVectorsForFile(relPath)
            )
              return;
            ...
            if (
              ...
              this.db
                .prepare("UPDATE files SET mtime = ?, size = ? WHERE path = ?")
                .run(stats.mtimeMs, stats.size, relPath);
              return;
            }
            ...
            if (e.code !== "ENOENT")
              console.warn(`Read error ${filePath}:`, e.message);
          }
        })
      );
      ...
      if (reusedChunkVectorCount > 0) {
        console.log(
          `[KnowledgeBase] ♻️ Reused ${reusedChunkVectorCount} chunk vector(s) from SQLite cache; skipped embedding for matching moved/copied content.`
        );
      }
      ...
      const checkTag = this.db.prepare(
        "SELECT id, vector FROM tags WHERE name = ?"
      );
      for (const t of uniqueTags) {
        const row = checkTag.get(t);
        if (row && row.vector)
          tagCache.set(t, { id: row.id, vector: row.vector });
        else {
...
for (const dName of updates.keys()) {
        ...
        const insertTag = this.db.prepare(
          "INSERT INTO tags (name, vector) VALUES (?, ?) ON CONFLICT(name) DO UPDATE SET vector = excluded.vector"
        );
        ...
        const updateFile = this.db.prepare(
          "UPDATE files SET checksum = ?, mtime = ?, size = ?, updated_at = ?, diary_name = ? WHERE id = ?"
        );
        const getFile = this.db.prepare(
          "SELECT id, diary_name FROM files WHERE path = ?"
        );
        const getOldChunkIds = this.db.prepare(
          "SELECT id FROM chunks WHERE file_id = ?"
        ); // 💡 新增
        const delChunks = this.db.prepare(
          "DELETE FROM chunks WHERE file_id = ?"
        );
        const delRels = this.db.prepare(
          "DELETE FROM file_tags WHERE file_id = ?"
        );
        const addChunk = this.db.prepare(
          "INSERT INTO chunks (file_id, chunk_index, content, vector) VALUES (?, ?, ?, ?)"
        );
        const addRel = this.db.prepare(
          "INSERT OR IGNORE INTO file_tags (file_id, tag_id, position) VALUES (?, ?, ?)"
        );
        const consumeMigrationCache = this.db.prepare(
          "DELETE FROM migration_deleted_files WHERE id = ?"
        );
          ...
          docs.forEach((doc) => {
            ...
            if (fRow) {
              ...
              if (fRow.diary_name !== doc.diaryName) {
                if (!deletions.has(fRow.diary_name))
                  deletions.set(fRow.diary_name, []);
            ...
            doc.chunks.forEach((txt, i) => {
              ...
              if (vectorSource) {
                // 🛡️ null 向量的 chunk 自然被跳过，不会写入错误数据
                const vecFloat =
                ...
                updates
                  .get(dName)
                  .push({ id: r.lastInsertRowid, vec: vecFloat });
              }
            });
      ...
      const { updates, tagUpdates, deletions, actualTagChanges } =
        transaction();
            ...
            chunkIds.forEach((id) => {
              try {
                ...
                if (e.message && !/not found|missing|absent/i.test(e.message)) {
                  console.warn(
                    `[KnowledgeBase] ⚠️ Failed to remove stale vector ${id} from "${dName}": ${e.message}`
                  );
                }
              }
            });
        ...
        try {
          ...
          if (e.message && e.message.includes("Duplicate")) {
            try {
              ...
              console.error(
                `[KnowledgeBase] ❌ Failed to upsert tag ${u.id}:`,
                retryErr.message
              );
            }
          }
          ...
          try {
            ...
            if (e.message && e.message.includes("Duplicate")) {
              ...
              try {
                ...
                console.error(
                  `[KnowledgeBase] ❌ Failed to upsert vector ${u.id} in ${dName}:`,
                  retryErr.message
                );
              }
            } else {
      ...
      console.log(
        `[KnowledgeBase] ✅ Batch complete. Updated ${updates.size} diary indices.`
      );

      // 优化1：数据更新后，检查是否需要重建矩阵（防抖 + 阈值）
      // 🌟 V7.2: 使用实际生成的 tag 共现对变动（以写入 file_tags 的行数为准）进行触发
      if (this.tagMemoEngine)
        this.tagMemoEngine.scheduleMatrixRebuild(actualTagChanges);
    } catch (e) {
      console.error(
        "[KnowledgeBase] ❌ Batch processing failed catastrophically."
      );
      ...
      if (this._isSqliteCorruptionError(e)) {
        ...
        batchFiles.forEach((f) => {
          ...
          if (count >= MAX_FILE_RETRIES) {
            console.error(
              `[KnowledgeBase] ⛔ File "${f}" failed ${MAX_FILE_RETRIES} times. Removing from queue permanently.`
            );
            this.pendingFiles.delete(f);
            this.fileRetryCount.delete(f);
          } else {
            ...
            console.warn(
              `[KnowledgeBase] ⚠️ File "${f}" retry ${count}/${MAX_FILE_RETRIES}.`
            );
          }
        });
      }
  ...
  async _handleDeleteBatch(filePaths) {
    ...
    try {
      ...
      const diaryByFileId = new Map(
        rows.map((row) => [row.id, row.diary_name])
      );
...
const insertMigrationFile = this.db.prepare(`
        ...
        for (const row of rows) {
          ...
          for (const chunk of chunks) {
            insertMigrationChunk.run(
              cacheRes.lastInsertRowid,
              chunk.chunk_index,
              chunk.vector
            );
          }
        ...
        const deleteFileTags = (ids) => {
          ...
          this.db
            .prepare(`DELETE FROM file_tags WHERE file_id IN (${placeholders})`)
            .run(...ids);
        };
        const deleteChunks = (ids) => {
          ...
          const placeholders = ids.map(() => "?").join(",");
          this.db
            .prepare(`DELETE FROM chunks WHERE file_id IN (${placeholders})`)
            .run(...ids);
        };
      ...
      let totalChunks = 0;
      for (const chunkIds of chunkIdsByDiary.values())
        totalChunks += chunkIds.length;

      if (rows.length > 1) {
        console.warn(
          `[KnowledgeBase] 🧹 Batched delete removed ${rows.length} file record(s), ${totalChunks} chunk vector(s).`
        );
      }
  ...
  _saveIndexToDisk(name) {
    ...
    try {
      if (name === "global_tags") {
        let stats = null;
        try {
          stats = this.tagIndex?.stats ? this.tagIndex.stats() : null;
        } catch (_) {}
        console.log(
          `[KnowledgeBase] 💾 Saving index start: ${name}, vectors=${
            stats?.totalVectors ?? "unknown"
          }`
        );
        if (this.tagIndex)
          this.tagIndex.save(
            path.join(this.config.storePath, "index_global_tags.usearch")
          );
      } else {
        ...
        if (idx && idx.save) {
          let stats = null;
          try {
            stats = idx.stats ? idx.stats() : null;
          } catch (_) {}
          console.log(
            `[KnowledgeBase] 💾 Saving index start: ${name}, vectors=${
              stats?.totalVectors ?? "unknown"
          ...
          idx.save(
            path.join(this.config.storePath, `index_diary_${safeName}.usearch`)
          );
        }
      }
      const elapsed = Date.now() - startedAt;
      console.log(
        `[KnowledgeBase] 💾 Saved index: ${name}, elapsed=${elapsed}ms`
      );
      if (elapsed > 5000) {
        console.warn(
          `[KnowledgeBase] 🧯 Slow synchronous index save detected: ${name}, elapsed=${elapsed}ms`
        );
      }
    } catch (e) {
      console.error(`[KnowledgeBase] Save failed for ${name}:`, e);
    }
  }
  ...
  _extractTags(content) {
    ...
    if (this.config.tagBlacklistSuper.length > 0) {
      const superRegex = new RegExp(
        this.config.tagBlacklistSuper.join("|"),
        "g"
      );
    ...
    if (uniqueTags.length > 50) {
      console.warn(
        `[KnowledgeBase] ⚠️ File has too many tags (${uniqueTags.length}). Truncating to top 50.`
      );
    ...
    for (const { diary_name } of allDiaries) {
      try {
        ...
        const dbCount = this.db
          .prepare(
            "SELECT COUNT(*) as count FROM chunks JOIN files ON chunks.file_id = files.id WHERE files.diary_name = ?"
          )
        ...
        if (diff > 10 && diff / (dbCount || 1) > 0.05) {
          console.warn(
            `[KnowledgeBase] ⚠️ Index/DB mismatch for "${diary_name}" (Index: ${stats.totalVectors}, DB: ${dbCount}). Rebuilding...`
          );
          // 标记为需要重建
        ...
        console.warn(
          `[KnowledgeBase] Ghost check failed for ${diary_name}:`,
          e.message
        );
      }
    }
  ...
  _startIdleSweep() {
    ...
    console.log(
      `[KnowledgeBase] 🧹 Idle index sweep started (TTL: ${Math.round(
        this.config.indexIdleTTL / 60000
      )}min, interval: ${Math.round(
        this.config.indexIdleSweepInterval / 60000
      )}min)`
    );
  }
  ...
  _evictIdleIndices() {
    ...
    if (this.config.idleSweepLogTick && this.diaryIndexLastUsed.size > 0) {
      console.debug(
        `[KnowledgeBase] 🧹 Idle sweep tick: tracked=${this.diaryIndexLastUsed.size}, loaded=${this.diaryIndices.size}`
      );
    }
    ...
    for (const [diaryName, lastUsed] of this.diaryIndexLastUsed) {
      ...
      try {
        ...
        evictedCount++;
        console.log(
          `[KnowledgeBase] 🧹 Evicted idle index: "${diaryName}" (idle ${Math.round(
            (now - lastUsed) / 60000
          )}min)`
        );
      } catch (e) {
        console.error(
          `[KnowledgeBase] ❌ Failed to evict index "${diaryName}":`,
          e.message
        );
      }
    }
    ...
    if (evictedCount > 0) {
      console.log(
        `[KnowledgeBase] 🧹 Idle sweep complete: evicted ${evictedCount} index(es), ${
          this.diaryIndices.size
        } remaining in memory, elapsed=${Date.now() - sweepStartedAt}ms.`
      );
    }
  }
