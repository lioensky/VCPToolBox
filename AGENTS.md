...
`main` 是本项目永远保持最先进、最新整合状态的主分支。

- `main` 应吸收已经确认进入项目主线的最新代码、文档和治理记录。
...
- 永远不要把 `prod/stable` 列入分支清理候选。
- 即使 `prod/stable` 已经被 `main` 吸收或显示为已合并，也只能视为受保护基线，不能视为可清理分支。
- 对 `prod/stable` 的任何合并、重置、强推、删除、重命名或保护规则变更，都必须先获得明确的当前回合人工授权。
